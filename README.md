...
- **쓰기 전 계약 승인**: 파일·설정·데이터를 쓰기 전에 목표·범위·변경 대상·결정·검증 방법·예상 영향 6항목 계약을 제시하고 사용자 승인을 받는다. 승인 전에는 어떤 쓰기도 시작하지 않는다.
- **완료 전 지식 영향 전수 판정**: 기획·설계·프로젝트 문서·CONVENTIONS·SECURITY·brain·memory·code-scan 8영역을 모두 `update` 또는 `no-op + 근거`로 판정한 뒤에만 완료 절차로 넘어간다.
- **사용자 최종 확인 전 종료 금지**: 계약 이행 결과와 8영역 판정을 제시하고, 사용자가 확인을 발화하기 전에는 완료를 선언하지 않는다.

상세 계약은 actor 축 SSOT `opal/core/references/harness/actor.md`와 `//oppm` 루프 정의 `opal/skills/opal-self-pm/SKILL.md`를 참조한다.
