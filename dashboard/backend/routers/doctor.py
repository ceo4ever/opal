...
@header {
  ...
  "layer": "router",
  "domain": "console",
  "description": "GET /api/doctor — doctor 4섹션+MCP+스킬 구조화 반환. project 파라미터 지원(캐시 키 분리 + 프로젝트 OPAL 구조 섹션 추가). tasks/ 체크는 **인자로 받은 그 경로**의 1-depth 리터럴 상태를 보고한다 — 루트 해석·단일 열거 함수 위임의 제외 대상이다(진단 도구 예외, 계약 원문 opal/core/references/harness/worktree.md §task root와 allocator root 계약). 핵심 파일 체크 대상 MEMORY.json 전환 + MEMORY.md 잔존 시 warn 노출 (078 F-009). 읽기 전용",
...
def _build_project_section(project_path: str) -> DoctorSection:
    ...
    # 소유권 기반 task root 해석을 씌우면 워크트리를 진단해도 그 태스크가 사는
    # 다른 루트를 보고해 「이 작업본에 태스크 문서가 없다」는 신호가 영구히
    # 사라진다 — 진단 도구가 진단 대상의 해석에 의존하는 순환이 된다.
    # 열거도 iter_task_dirs로 갈지 않는다:
