...
#: worktree registry 디렉토리 — 발급원은 worktree-tool이고 Console은 읽기만 한다.
_WORKTREE_REGISTRY_DIR = os.path.join(".opal-worktrees", ".meta")


def _active_worktree_task_roots(project_path: str) -> list[str]:
    """registry에 등록된 active worktree의 tasks root realpath 화이트리스트.

    태스크 캡슐의 거처는 허브 고정이 아니라 루트 소유권으로 정해진다
    (`opal/core/references/harness/worktree.md` §task root와 allocator root 계약).
    그래서 Console이 워크트리 소유 태스크를 열려면 허용 루트를 하나 더 알아야
    하는데, 그 목록의 **발급원은 worktree-tool의 registry metadata**다 — Console은
    cwd나 `.opal-worktrees` 문자열로 경로를 추측하지 않고 발급값만 읽는다.

    각 슬롯 메타의 `task_path`는 불변식 `task_path == realpath(task_home/tasks/
    task_folder)`를 만족하므로, 그 부모가 곧 그 작업본의 tasks root다.
    `task_ownership_version` 이전에 만들어진 legacy 슬롯은 `task_path`가 없으므로
    조용히 건너뛴다 — legacy 태스크는 허브 task path를 그대로 쓴다.

    읽기 전용이며, 읽을 수 없는 registry(부재·권한·깨진 JSON)는 빈 목록이다 —
    비워크트리 실행에서는 항상 빈 목록이므로 resolver 동작이 달라지지 않는다.
    """
    import json

    registry_dir = os.path.join(project_path, _WORKTREE_REGISTRY_DIR)
    try:
    ...
    for name in entries:
        if not (name.startswith("task_") and name.endswith(".json")):
            continue
        try:
            with open(os.path.join(registry_dir, name), encoding="utf-8") as f:
                meta = json.load(f)
        except (OSError, ValueError):
            continue
        ...
        if not task_path or not isinstance(task_path, str):
            continue
        tasks_root = os.path.dirname(os.path.realpath(task_path))
        if tasks_root not in roots and os.path.isdir(tasks_root):
...
def get_task_detail(
    ...
    # 허용 루트는 허브 tasks/ + registry 등록 active worktree의 tasks root뿐이며,
    # 화이트리스트 밖 경로는 resolver가 거부한다(비워크트리에서는 빈 목록).
    task_dir = resolve_task_dir(
        project_path, task_id,
        extra_task_roots=_active_worktree_task_roots(project_path),
    )
    if task_dir is None:
