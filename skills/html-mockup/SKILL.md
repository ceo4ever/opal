...
| OPAL 프로젝트 (태스크 폴더 없음) | `STATE.md`, `MEMORY.json` | 진행 중 컨텍스트 |
...
1. `~/.opal/skills/interview/SKILL.md`
2. (미존재 폴백) 인라인 폴백 — 저장 위치 1문 + 핵심 항목 1~2문으로 최소 인터뷰 수행

**7개 인터뷰 노드 (의사결정트리 순서 — interview SKILL.md 원칙 준수: 매 콜당 1문씩, 직전 답변으로 다음 노드 분기)**:
...
진행 순서: 위 표의 1→7 순으로 **한 번에 1문씩** 질문하되, 각 노드의 스킵 조건에 걸리면 질문 없이 1줄 통지 후 다음 노드로 넘어간다. 컨텍스트 자동 흡수가 충분하면 1번(저장 위치, 필수)만 묻고 나머지는 전부 스킵하여 종결 가능.
...
`~/.opal/skills/interview/SKILL.md`가 없을 때:
...
한 번에 1문씩, 총 2~4문으로 최소 인터뷰 수행. AskUserQuestion 도구 직접 사용.
