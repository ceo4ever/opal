...
description: |
  **코드베이스 분석 및 기술 컨텍스트 수집 단계 스킬**. TASK.md를 기반으로 기존 코드를 분석하고, 기술 스택을 식별하여 추천 스킬/MCP를 매핑한다.
  반드시 이 스킬을 사용해야 하는 상황: 오케스트레이터(dtp-dev)가 ANALYSIS 단계를 디스패치할 때.
  필수 입력: TASK.md. 선택 입력: 프로젝트 docs/. 보장 출력: ANALYSIS.md.
---

# dtp-analysis — 코드베이스 분석 및 기술 컨텍스트 수집

## 실행 컨텍스트

- **호출자**: 오케스트레이터(dtp-dev)가 ANALYSIS 단계를 디스패치
...
```
Read ~/.opal/skills/dtp-analysis/personas/application-architect.md
```
...
```
Read ~/.opal/skills/dtp-analysis/references/tech-context-guide.md
```
...
```
Read ~/.opal/skills/dtp-analysis/references/analysis-guide.md
```
