...
- 스킬: dtp-wireframe, 입력: TASK.md + 정책서/이미지
- 완료 → dtp-qa 호출 (단계: WIREFRAME) → 사용자 보고
...
- 스킬: dtp-execute, checklist_source: wireframe.md
- **UI 구현 모드**: ui-designer scaffold(프로토) 또는 plan-driven(프로덕션) 호출

### 완료 후
1. dtp-qa 호출 (단계: EXECUTE-UI) → 빌드/린트 + wireframe↔코드 대조
