...
description: |
  **와이어프레임 생성 단계 스킬**. TASK.md와 입력물(정책서/이미지/구두 요청)을 기반으로 wireframe-builder 스킬에 위임하여 wireframe.md를 생성한다.
  반드시 이 스킬을 사용해야 하는 상황: 오케스트레이터(dtp-dev-wf)가 WIREFRAME 단계를 디스패치할 때.
  필수 입력: TASK.md + 입력물. 보장 출력: wireframe.md.
---

# dtp-wireframe — 와이어프레임 생성

## 실행 컨텍스트

- **호출자**: 오케스트레이터(dtp-dev-wf)가 WIREFRAME 단계를 디스패치
...
```
Read ~/.opal/skills/dtp-wireframe/personas/service-planner.md
```
