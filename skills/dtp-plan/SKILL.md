...
description: |
  **구현 계획 수립 단계 스킬**. TASK.md와 ANALYSIS.md(선택)를 기반으로 실행 가능한 구현 청사진을 작성한다. ANALYSIS.md 유무에 따라 분석 깊이가 자동 조절된다.
  반드시 이 스킬을 사용해야 하는 상황: 오케스트레이터(dtp-dev, dtp-dev-short)가 PLAN 단계를 디스패치할 때.
  필수 입력: TASK.md. 선택 입력: ANALYSIS.md. 보장 출력: PLAN.md, execution-plan.json (FE/BE 시).
---
...
### Step 5: execution-plan.json 생성 (FE/BE 작업 시)
