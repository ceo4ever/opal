---
name: opal-pilot-write
description: |
  **범용 문서 작성 오케스트레이터**. 코드 변경 없이 단일 문서를 체계적으로 작성하는 3단계 파이프라인.
  반드시 이 스킬을 사용해야 하는 상황: "opal-pilot-write", "opw", "문서 작성해줘", "보고서 작성", "가이드 만들어줘", "기획서 써줘", "명세서 작성", "설계서 작성", "정책서 작성", "PRD 작성".
  코드 구현이 수반되면 opal-pilot-dev/opal-pilot-dev-short, 프로젝트 파일럿은 opdp, 와이어프레임은 opal-pilot-dev-wireframe, API 분석은 api-analyzer.
---
...
**아닌 것**: 코드 구현(opal-pilot-dev), 프로젝트 파일럿(opdp), 와이어프레임(opal-pilot-dev-wireframe), API 분석(api-analyzer)
...
QA: 복잡한 문서는 **QA Gate** (op-task-qa) 호출, 단순 문서는 스킵.
