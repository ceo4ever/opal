# TASK: opal-project-dev-pilot 스킬 개발
...
개발 프로젝트의 전체 라이프사이클을 관리하는 `opal-project-dev-pilot` (약식: `//opdp`) 스킬을 만든다. opi가 프로젝트를 셋업한 후, 캡틴이 "만들어줘"라고 요청했을 때 PRD/TRD 작성 → 로드맵 수립 → 태스크 순차 실행까지 이어지는 흐름을 구현한다.
...
- [ ] 우선순위, 의존성 정리
- [ ] PM이 검수 → 태스크 분할 적절성/누락 확인 → PM 통과 후 캡틴 검토 요청
- [ ] 캡틴 확정

### R5. 태스크 순차 실행 (Phase 4)
- [ ] 로드맵 순서대로 otp 스킬 실행
- [ ] 각 태스크 완료 시 PM 검수 (검토 기준 + 참조 문서 반영 확인)
- [ ] PM 통과 후 다음 태스크 진행
- [ ] 진행 상황 추적 및 로드맵 갱신
- [ ] 캡틴에게 정기 보고 (태스크 완료 시)

### R6. PM 검수 역할
- [ ] 모든 Phase의 산출물을 PM이 먼저 검수한다
- [ ] 검수 흐름: 워커/알투 작성 → PM 검수 → 미달 시 추가 지시 → 통과 시 캡틴 검토 요청
- [ ] 캡틴은 PM이 통과시킨 결과물만 검토한다 (품질 필터링)
- [ ] .opal/AGENT.md의 PM 검토 기준 + 참조 문서 전달 의무 적용
- [ ] PM 학습 루프 — 판단 불확실 시 캡틴에게 질문 → 확정 기준 기록

### R7. opi 연동
- [ ] opi 완료 후 원래 요청이 개발인 경우 opal-project-dev-pilot 자동 호출
...
opi: 인터뷰 → 프로젝트 셋업
  ...
  │
  ▼
opal-project-dev-pilot 자동 호출 (//opdp):
  │
  │  Phase 1: PRD 작성
  │    캡틴과 대화 → PRD 초안
  │    → PM 검수 (요구사항 완전성, 우선순위 적절성)
  │    → 미달 시 수정 지시 → 재작성
  │    → PM 통과 → 캡틴 검토 요청
  │    → 캡틴 확정 → docs/PRD.md
  │
  ...
  │
  │  Phase 4: 태스크 순차 실행
  │    T1 → otp 실행 → PM 검수 → 다음 태스크
  │    T2 → otp 실행 → PM 검수 → 다음 태스크
  │    ...
  │    (각 태스크 완료 시 캡틴에게 진행 보고)
  │
  ▼
전체 완료 보고
```
...
PM 검수
  ...
  │  참조 문서 반영 확인 (PRD, TRD 등)
  │  프로젝트 원칙/기준 부합 확인
  │
  ├─ 미달 → 추가 지시 (구체적 피드백) → 재작성 → 재검수
  │
  └─ 통과 → 캡틴 검토 요청
              │
              ├─ 캡틴 피드백 → 반영 → 재검토
...
캡틴은 PM이 필터링한 결과물만 본다. 품질이 낮은 초안이 캡틴에게 올라가지 않는다.

## OPAL 스킬 체계에서의 위치

```
//opi                프로젝트 WHAT/WHY 정의 + 셋업
//opdp                PRD/TRD + 로드맵 + 태스크 순차 실행 + PM 검수   ← 이 스킬
