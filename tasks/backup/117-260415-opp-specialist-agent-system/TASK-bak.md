...
**변경 후**: 문서 선별 → 에이전트별 슬라이싱 → 각 전문 에이전트에 분리 주입

| 단계 | 현재 | 변경 |
|------|------|------|
| Step 2. 문서 선별 | 작업 전체 기준 | 영역별(FE/BE/DB) 분류 추가 |
| Step 3. 핵심 제약 추출 | 워커 1개에 전부 주입 | 에이전트별 관련 제약만 주입 |
| Step 4. 종속 관계 | 문서 간 종속만 | Step 간 의존 → Batch 구성 추가 |
| 워커 컨텍스트 주입 | 템플릿 1개 | 에이전트별 템플릿 (FE/BE/범용) |
...
Step 6. 실행 라우팅 (신규):
1. PLAN.md §4 실행 체크리스트에서 각 Step의 영역 태그를 파싱
2. 영역 → 에이전트 매핑: FE → opal-fe-agent, BE → opal-be-agent, DB → opal-be-agent (또는 opal-db-agent), 공통/환경/배치 → opal-task-agent
3. 의존 그래프 기반 Batch 구성
4. Batch 내 독립 Step → 병렬 / 의존 Step → 순차
...
- [ ] **R-3**: opal-plan-agent AGENT.md 작성
  - **무엇을**: PLAN 단계 전문 워커 에이전트 정의. 코드 분석 + 설계 + 테스트 시나리오 작성을 고품질로 수행한다. model: advanced 고정. 전체 docs/를 읽을 수 있으며, 기능별 분석·설계·병렬/순차 분류·에이전트 라우팅을 수행한다.
  - **어디에**: `opal/agents/opal-plan-agent/AGENT.md`
  - **왜**: PLAN 단계는 전체 프로젝트 컨텍스트가 필요하며, advanced 모델로 고품질 설계를 보장
  - **AC**: AGENT.md가 OPAL 에이전트 규격을 따른다. model: advanced가 명시되어 있다. PLAN.md 작성 시 §4 실행 체크리스트에 agent 필드를 포함하도록 지시되어 있다.
...
- [ ] **R-5**: op-dev-plan SKILL.md에 agent 필드 추가
...
- [ ] **R-6**: op-dev-plan SKILL.md에 docs/ 갱신 Step 자동 생성 규칙 추가
...
- [ ] **R-7**: opal-pm.md §3 Step 6~7 신규 추가 (실행 라우팅 + 컨텍스트 슬라이싱)
  - **무엇을**: 디스패치 전 프로세스(§3)에 Step 6(실행 라우팅)과 Step 7(컨텍스트 슬라이싱)을 추가한다. Step 6에서는 PLAN.md §4의 agent 필드를 파싱하여 에이전트별 배치(Batch)를 구성하고, Step 7에서는 에이전트별로 필요한 컨텍스트(PLAN.md §3 해당 F-NNN, 도메인 문서, TEST-SCENARIO 해당 TS-ID, 선행 Batch changed_files)만 추출한다.
  - **어디에**: `opal/core/references/opal-pm.md` — §3 디스패치 전 프로세스
  - **왜**: PM의 디스패치 방식을 "전체 문서 주입"에서 "에이전트별 슬라이싱 주입"으로 전환하여 토큰 효율 극대화
  - **AC**: §3에 Step 6(실행 라우팅: 영역→에이전트 매핑, 의존 그래프→Batch 구성, 병렬/순차 판별)이 정의되어 있다. §3에 Step 7(컨텍스트 슬라이싱: 에이전트별 필요 컨텍스트 추출)이 정의되어 있다. 에이전트별 문서 매핑 테이블이 포함되어 있다.

- [ ] **R-8**: opal-pm.md §4 검토 게이트 항목 추가 (영역 침범 + 인터페이스 정합성)
...
- [ ] **R-9**: opal-pm.md §6 참조 문서 전달 의무 변경 (에이전트별 문서 매핑)
...
- [ ] **R-10**: opal-pm.md §10 통합 조율 신규 섹션
...
- [ ] **R-11**: opal-pm.md §4 PM Gate에 docs/ 무효화 체크 항목 추가
...
- [ ] **R-12**: 관련 스킬 SKILL.md에서 에이전트 참조 갱신
  - **무엇을**: opal-task-agent를 참조하는 스킬(op-dev-execute, op-dev-analysis, op-dev-plan, op-dev-test-scenario)에서 전문 에이전트 사용 안내를 추가한다. "PM이 REGISTRY 또는 PLAN.md agent 필드에 따라 적합한 에이전트를 선택한다"로 갱신. 기존 opal-task-agent 참조는 폴백으로 유지.
  - **어디에**: 해당 스킬 SKILL.md의 실행 주체 섹션
  - **왜**: 전문 에이전트 체계 도입을 스킬 레벨에서도 반영
  - **AC**: 각 스킬 SKILL.md의 실행 주체에 "전문 에이전트 또는 opal-task-agent (폴백)"이 명시되어 있다. 기존 동작(opal-task-agent)이 폴백으로 유지된다.

- [ ] **R-13**: agents.md 레지스트리에 전문 에이전트 등록
  - **무엇을**: `opal/core/references/agents.md`에 전문 에이전트 4종(opal-fe-agent, opal-be-agent, opal-plan-agent, opal-test-agent)을 등록한다.
  - **어디에**: `opal/core/references/agents.md`
  - **왜**: 에이전트 레지스트리에서 전문 에이전트를 관리하기 위함
  - **AC**: 4종 에이전트가 agents.md에 등록되어 있다. 기존 범용 에이전트와의 관계(폴백 구조)가 설명되어 있다.

### 5. 에이전트 디렉토리 이동

- [ ] **R-14**: OPAL 전용 에이전트를 agents/ → opal/agents/로 이동
...
- `opal/skills/op-dev-plan/SKILL.md` — PLAN 스킬 (R-5, R-6)
- `opal/skills/op-dev-execute/SKILL.md` — EXECUTE 스킬 (R-12)
...
| 2 | PLAN.md에 agent/영역 기반 라우팅 | R-5, R-6 | PM 디스패치 자동화의 기반 |
| 3 | PM 컨텍스트 슬라이싱 로직 | R-7, R-8, R-9 | 토큰 효율 극대화 |
| 4 | opal-plan-agent 분리 | R-3 | PLAN 품질 안정화 |
| 5 | opal-test-agent 강화 | R-4 | 도메인별 테스트 전략 분리 |
| 6 | PM 통합 조율 + docs/ 갱신 | R-10, R-11 | 전문 에이전트 체계 전제 조건 |
| 7 | 스킬/레퍼런스 갱신 | R-12, R-13 | 일관성 보장 |
| 8 | 에이전트 디렉토리 이동 | R-14 | 구조 정리 |
