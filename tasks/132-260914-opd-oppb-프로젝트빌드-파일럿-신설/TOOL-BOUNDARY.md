...
## 8. 알려진 한계 — Checkpoint 쓰기 귀속의 쓰기 순서 역전 (lease 밖 **소스** 경로 한정)

| # | 대상 | 한계 | 근거·처리 |
|---|---|---|---|
| L-1 | `opal/tools/oppb-runtime-tool/checkpoint.py` `classify_writes` — **lease 밖 소스 경로 한정** | attempt가 **lease 밖 소스를 먼저 쓰고 자기 lease를 나중에 쓰면** `foreign_mtime < own_marker`가 되어 lease 밖 쓰기로 귀속되지 않는다. **공유 지식(MEMORY·brain) 축에는 이 한계가 없다** — 그 경로는 귀속 판정 이전에 분리되어 dirty 존재만으로 무조건 거부된다. | 소스 경로는 타 Runner가 자기 lease에서 정당하게 쓸 수 있어 귀속 판정이 필요하고(S-13①), 거기서만 순서 역전이 남는다. 구현 결함이 아니라 **mtime 단독 관측의 근본 한계**다. 그 파일시스템 상태는 "다른 Runner가 자기 lease에서 작업 중인 정상 동시 실행"(S-13①)과 완전히 동일하고 차이는 관측되지 않는 쓰기 순서뿐이라, 시작·종료 baseline 봉인으로도 구별되지 않는다. attempt별 격리 worktree는 제안서가 명시 거부하고(worktree 1개), Runner 자기 신고는 §4.5의 사후 탐지 요구와 어긋난다. **v1은 이 한계를 감수한다.** 쓰기 주체를 직접 관측할 수단(파일시스템 감사 이벤트 등)이 생기면 mtime 순서 비교 전체가 교체 대상이다. 공유 지식 경로는 Runner가 정당하게 쓰는 경우가 없으므로(제안서 §4.3 capability agent 행동 계약, §5 P5 지식 반영 1회) 귀속 없이 닫았다 — `test_s12_3`(정순)·`test_s12_3a`(역순)이 두 방향을 모두 고정한다. 같은 내용이 `classify_writes` docstring에 SSOT로 기재돼 있다. |
