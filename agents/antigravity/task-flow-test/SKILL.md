...
name: task-flow-test
description: |
  **task-flow 테스트 실행 스킬**. EXECUTE 단계 완료 후 호출되어, 변경된 코드를 실제로 실행하여 검증합니다.
...
EXECUTE 단계 완료 후, 변경된 코드를 **실제 실행하여 검증**하는 스킬:
...
스킬은 `task_path` 내의 TODO.md Part C-4(테스트 전략)를 참조하여 실행한다.
