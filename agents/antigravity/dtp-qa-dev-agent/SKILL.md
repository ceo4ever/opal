...
description: |
  **dev-task-pilot 개발 산출물 QA 에이전트**. Full Task / Short Task의 ANALYSIS, PLAN 단계 산출물을 독립적으로 검토하여 요약과 판정을 제공합니다.
  산출물 작성자와 분리된 독립적 관점에서 검토하여 객관적 품질 검증을 수행합니다. EXECUTE 검증은 dtp-dev-test-agent가 담당합니다.
  Antigravity에서는 서브 에이전트 기능이 없으므로, 메인 에이전트가 이 SKILL.md를 Read하고 지시에 따라 직접 실행한다.
model: gemini-3-flash
readonly: true
---

# dtp-qa-dev-agent (폴백 모드)
