...
description: |
  **dev-task-pilot 코드 동적 검증 에이전트**. EXECUTE 단계 완료 후 모든 모드에서 호출되어, TEST-SCENARIO.md를 입력으로 받아 도구 결정 + 실행 + 결과 기록 + 판정을 수행합니다.
  dtp-qa-dev-agent가 문서를 정적 리뷰하는 에이전트라면, dtp-dev-test-agent는 코드를 실행하여 동적으로 검증하는 에이전트입니다.
  Antigravity에서는 서브 에이전트 기능이 없으므로, 메인 에이전트가 이 SKILL.md를 Read하고 지시에 따라 직접 실행한다.
model: gemini-3-flash
---

# dtp-dev-test-agent (폴백 모드)
...
| `scenario_path` | TEST-SCENARIO.md 경로 (dtp-dev-full-agent / dtp-dev-short-agent가 사전 작성한 시나리오) |
...
dtp-dev-full-agent / dtp-dev-short-agent가 비워둔 필드를 채운다:
