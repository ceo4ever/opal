...
- (근거: task:104 PLAN §2.4·§2.6 M-3) opdd 역공학 트랙에서도 같은 패턴이 재현됐다 — 개념 ERD·개념 모델링 산출물은 신규(greenfield) 트랙에서만 만들어지고 역공학 트랙에서는 애초에 만들지 않는다. 이 조건부 산출물을 `model.pm_gate.gate.checklist`의 자유 문언이 아니라 `gate.artifacts`(존재를 기계적으로 확인하는 배열)에 넣었다면, 역공학 트랙 태스크는 존재하지 않는 것이 정상인 산출물 때문에 영구히 게이트를 통과하지 못했을 것이다. `gate.artifacts`는 빈 배열로 유지하고, 트랙 조건은 사람이 읽는 `checklist` 문언으로만 표현해 이 위험을 피했다.
...
- 트랙·모드에 따라 산출물 유무가 갈리는 파이프라인 설계 전반에 이 판별 기준이 반복 적용된다(task:091 컨벤션 보고서, task:104 opdd 개념 모델링 산출물) — 조건부 산출물을 발견하면 게이트가 아니라 체크리스트로 보내는 것이 일회성 예외가 아니라 반복되는 설계 규칙이다.
