---
type: concept
title: oppl 루프 액션 에이전트 위임 구조 — 태스크 단위 컨텍스트 격리
tags:
- oppl
- executor
- delegation
...
기존 oppl은 생성자(T1 설계+T2 시나리오) → Evaluator(G) → 생성자 재개(T3) → test-agent(T4a) 순으로 PM이 직접 지휘하는 "하이브리드 C" 구조였다. 백로그가 태스크 10~20개 규모면 PM 컨텍스트가 포화되고, 플랫폼 자동 요약(compaction)이 태스크 진행 중간에 발생하면 판단 품질이 저하되는 문제가 있었다 (근거: task:065 TASK.md §배경). oppd(`opal-task-action-agent`)·opsdd(`opal-sdd-action-agent`) 선례는 중간층 액션 에이전트가 하위 워커를 재디스패치하고 PM은 디스패치 1회 + 결과 1건만 받는 구조가 이미 검증되어 있었다.
...
- **blocked 반환 트리거 7종**: 비가역 행동 요구, 에스컬레이션 대상, 계약 갱신 필요 drift, 무진전 감지, 반복 상한 초과, 하드블로커(순서 역전·SSOT 손상·readonly 위반), decision_required(용어 불일치). 루프 액션 에이전트는 소유자에게 직접 에스컬레이션하지 않고 PM이 수행한다.
- **결과 계약 6필드**: `{task_id, verdict, scenario_results, changed_files, done_md_path, blockers}`.
- **루프 액션 에이전트 직접 갱신 금지 대상**: STATE.md(PM에게 위임), CONTRACT.md(계약 갱신 drift는 blocked).

## 영향·관계

`opal/agents/opal-loop-action-agent/AGENT.md`(신규 루프 액션 에이전트 정의), `opal/skills/opal-pilot-project-loop/SKILL.md`(태스크 내부 파이프라인·디스패치 절이 루프 액션 에이전트 위임 구조로 개편, v1.2), `opal/skills/opal-pilot-project-loop/references/loop-control.md`(§3 예산 관찰 단위가 "루프 액션 에이전트 1회 디스패치" 기준으로 정합, v1.1), `opal/skills/opal-pilot-project-loop/references/contract.md`(§4에 루프 액션 에이전트 CONTRACT drift 경계 문단 추가, v1.1)에 영향을 준다. `opal/skills/opal-pilot-project-loop/references/verification.md`는 순서 불변 규칙이 주체 중립(PM/루프 액션 에이전트 동일 적용)이라 무변경이다. `docs/PROJECT.md`/`docs/ARCHITECTURE.md`의 Project Loop 컴포넌트 표·구조도에도 루프 액션 에이전트 계층이 반영되었다.

- [[opal-loop-action-agent]] — 이 결정으로 신설된 루프 액션 에이전트 엔티티.
- [[oppl-two-loop-orchestrator]] — 루프 액션 에이전트를 디스패치하는 상위 오케스트레이터.
