...
자가검토가 이 행을 `promote_candidates`로 표면화했으나 **졸업시키지 않는다.**

- **규칙 축은 이미 졸업했다** — "수기 누적 이력 절을 만들지 않는다"가 `.opal/AGENT.md`(업무지침·금지사항 2개소)와 `opal/core/references/opal-doc-standard.md` §5에 규범으로 들어가 있다.
- **실행 축은 미완이다** — 137파일 일괄 제거와 동반 개정 8지점은 착수되지 않았다. 실측: `opal/skills/opal-workspace-sync/SKILL.md`에 `## 변경이력` 표가 남아 있다.

행을 지우면 이 실행 백로그가 사라진다. 규칙만 남고 정리는 안 된 상태가 고착되므로 `active`로 유지한다.
