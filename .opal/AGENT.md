...
- [ ] 문서 표준·@header 등 추적 가능성 규칙을 따랐는가
...
- **문서 표준 준수**: git 관리 문서에는 수기 누적 이력 절을 만들지 않는다. 기준은 프로젝트 소스의 `opal/core/references/opal-doc-standard.md` §4~§5가 소유한다.
...
- **수기 누적 이력 절 생성 금지** — 스킬·에이전트·참조 문서와 git 관리 프로젝트 문서의 이력은 git/task 기록이 소유한다.
