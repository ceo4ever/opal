...
|----------|------|------|------|---------|---------|
| 2026-05-10 | OPAL 보안 강화 — SECURITY.md 8섹션 신설 + High 4(GC-001~004) + Medium 핵심 3(GC-006/007/010) fix. v0.4.0 보안 진단 14건 중 본 태스크 범위 9건 처리 (Low/일부 Medium 후속 분리). 신규 1 + 수정 14 = 15 파일. 캡틴 결정 SSOT: W-1 ReDoS MAX_DOTSTAR > 2 (react-components 거짓양성 방지) / P-D-9 community-skills-registry.json schema_notes 갈음 / P-D-10 OPAL_ALLOW_FORK 옵트인 / P-D-11 anthropics commit_sha:null 명시. registry v2 → v2.1 minor bump (skill-registry.js 양쪽 인식). install 무결성: release tag + sha256sums 부재 시 prompt/거부 + main UNVERIFIED banner. MCP spawn 화이트리스트(npx/npm/node/python3) + fork repo banner. ReDoS isUnsafeRegex (길이 100 / dotStar > 2 / nested quantifier) + 입력 256자 + path.resolve homedir. OPAL_HOME 가드 3 위치. mac 회귀 검증 캡틴 통과. 후속: Windows 회귀(push 후) / commit_sha 실 SHA / install.ps1 CLM 보호 / 143 push (f43e56a local) / release.yml 자동 publish 미동작 진단 (144) | 완료 | tasks/144-260510-opp-security-hardening/ | 2026-05-10 20:23 | 2026-05-10 23:20 |
