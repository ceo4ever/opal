...
| | opal-pilot-dev Short profile (opds) | Short Task 기본 진입점. 별도 물리 `opal-pilot-dev-short` 없이 canonical `opal-pilot-dev`가 `skill=opds`와 Short pipeline을 선택 |
...
| **SDD 내부 단계** | opal-pilot-sdd/internal-skills/op-sdd-spec | SPEC 단계 — SDD 명세 작성 (`opal-pilot-sdd` 내부 디스패치 전용) |
| | opal-pilot-sdd/internal-skills/op-sdd-plan | SPEC-PLAN 단계 — SDD 구현 계획 수립 (`opal-pilot-sdd` 내부 디스패치 전용) |
| | opal-pilot-sdd/internal-skills/op-sdd-action-plan | ACT 전용 경량 PLAN — SPEC.md + SPEC-PLAN.md + TEST-SCENARIOS.md + ACT 정의 기반 (`opal-sdd-action-agent` 내부) |
...
─────────────────                  ──────────────────
skills/* (독립 8개) ──┐
opal/skills/* (38개)──┼─ install ─→  ~/.opal/skills/
...
│   ├── skills/                          OPAL 스킬 (38개)
│   │   ├── opal-pilot-dev/              오케스트레이터: Full profile (opd) + Short profile (opds)
...
│   │   │   └── internal-skills/          SDD 내부 단계 스킬 3종
