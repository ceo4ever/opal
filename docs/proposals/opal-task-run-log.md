...
3. `AGENTIC-LOG.md`의 판단·오류·수정·에스컬레이션 기록은 표준 이벤트로 흡수하고 active 신규 태스크에서 폐지한다.
4. `.oppl-run/`은 플랫폼 원본 증거로 유지하되 표준 로그로 단방향 변환한다.
5. 워커 완료는 채널별 실증된 completion profile에 따라 adapter가 기계적으로 관측한 사건으로 인정한다.
...
9. `observed_trajectory`에서 activity와 terminal은 서로 다른 시점에 adapter가 관측한 증거여야 하며 하나의 완료 알림을 둘로 분해하지 않는다.
10. 전체 도입은 Agent 채널 capability spike → core shadow → profile별 enforcement → 나머지 pilot 확산으로 나눈다.
11. 중간 사건을 관측할 수 없는 채널에는 강한 궤적 게이트를 주장하지 않는다. 시작·종료만 기계
    관측되면 약한 terminal profile, 그것도 불가능하면 shadow를 유지한다.
...
| `run/.runtime/run-log-{run_id}.sqlite3` | 없음 | 순번·멱등 키 index와 worker capability registry | 신규, gitignore |
...
| `worker.capability.issued` | `run-log-tool` | direct 기록 capability 발급 시 | token verifier·scope·만료의 복구 원천 |
| `worker.capability.revoked` | `run-log-tool` | terminal 전 명시 폐기 시 | 조기 capability 폐기 원천 |
| `activity` | worker/PM | `observed_trajectory` 정상 완료 워커마다 trusted adapter 사건 1건 이상 | 의미 있는 진행·판단·검증·재시도 |
...
채널은 Phase 0의 실제 adapter 증거로 다음 completion profile 중 하나를 가진다.

| profile | 기계 관측 가능 범위 | 정상 완료 게이트 | 허용 mode |
|---|---|---|---|
| `observed_trajectory` | start + 중간 message/stream/tool + terminal | trusted started + activity 1건 이상 + terminal | shadow·active |
| `observed_terminal` | start + terminal만 | trusted started + terminal + 기존 artifact/PM 검증 | shadow·active |
| `cooperative` | adapter 기계 증거 없음 | 완료 차단에 관여하지 않음 | shadow만 |

두 active profile의 공통 조건은 같은 `worker_run_id`의 started 1건, terminal 정확히 1건,
terminal 결과 또는 원인, `duration_ms`와 `duration_source=adapter_monotonic`이다. shadow는 duration
대신 `duration_unknown_reason`을 허용한다. `observed_terminal`은 중간 궤적이 없음을 명시적으로
노출하며 R-2의 강한 궤적 달성으로 계산하지 않는다. 표의 기존 artifact/PM 검증은
`state-tool.check_gate_artifacts()`가 검사하는 `task_steps[].gate.artifacts`와 PM Gate를 뜻한다.
artifact 검사는 `--force + --note`로 우회 가능하고 decision 로그에 `gate_artifact_force`가 남으므로,
이 profile의 무조건적인 기계 강제 범위는 trusted started + terminal까지다.

`observed_trajectory`의 activity는 단순 heartbeat가 아니라 결과에 영향을 준 작업 경계를 기록한다. 짧은 작업도
“대상 확인 → 변경 또는 판정 완료”를 1건으로 남긴다. active 완료 게이트에 기여하는 activity는
adapter가 완료 알림 전에 실제 수신한 별도 message·stream·tool/process 사건이어야 한다. worker의
자발적 direct activity는 보조 궤적으로 보존하지만 완료 게이트에는 기여하지 않는다. 이 조건은
...
|---|---|---|---|---|
| 워커 직접 기록 | `worker` | `direct / worker` | `worker_log_token_id` + 서로 다른 호출의 source ID/hash/observed_at | 불가·보조 궤적만 |
...
trusted adapter event는 `actor.kind=worker`, `provenance.type=adapter`, 그리고
`provenance.recorded_by.kind=adapter`를 모두 만족한 사건이다. adapter 경로는 원본 메시지·도구·
프로세스 결과의 ID, hash, 관측 시각이 검증돼야 한다. `provenance.type=direct|import`는 actor가
...
- 중간 메시지·stream·tool 사건을 관측할 수 없지만 start·terminal은 관측 가능하면
  `observed_terminal`만 허용한다. start·terminal도 기계 관측할 수 없는 채널은 `cooperative`로
  shadow를 유지한다. worker direct activity 준수율은 어느 profile에서도 승격 근거가 아니다.
...
| `source.kind` | `worker_event | process_start | agent_handshake | agent_message | stream_event | tool_result | process_exit | agent_error | legacy_line | oppl_event` | type별 허용 조합을 schema `if/then`으로 검사 |

- `worker.started/completed/failed/blocked`의 `actor.kind`는 `worker`만 허용한다.
- `worker.capability.issued/revoked`의 actor와 recorded_by는 `tool`만 허용한다.
...
- direct worker append는 호출자가 재시도에도 유지하는 `request_id`가 필수다. source ID는 이 ID에서
  결정론적으로 파생하고 SHA-256은 정규화한 제출 payload로 계산한다.
- `provenance.type=adapter`는 source ID·SHA-256·`observed_at`, import는 source ID·SHA-256·`locator`가 필수다.
- active `worker.started`는 `process_start | agent_handshake`, activity는
  `agent_message | stream_event | tool_result`, terminal은 `process_exit | agent_error`만 허용한다.
- adapter append의 `request_id`는 upstream source ID에서 결정론적으로 만들고, state-tool outbox
  사건은 사전에 확정한 event ID를 request ID로 사용한다.
- import는 `recorded_by.kind=tool`만 허용하고 trusted adapter 판정에서 항상 제외한다.
- 같은 `(run_id, request_id)`의 동일 payload 재호출은 기존 event를 반환하고, 다른 payload 재사용은
  `request_id_conflict`로 거부한다. 응답 유실 뒤 재시도도 terminal 중복을 만들지 않는다.
...
- `duration_ms`는 동일 프로세스 안의 monotonic span 합으로 측정하고 timestamp 차분으로 만들지 않는다.
- `duration_source`는 `adapter_monotonic | worker_monotonic | unknown`이다. active terminal은
  `adapter_monotonic`만 허용하며 adapter가 관측한 process start~terminal 구간을 측정한다.
- 동일 `worker_run_id` resume가 프로세스 경계를 넘으면 adapter가 각 process span을 별도로 측정해
  합산한다. direct worker의 `worker_monotonic` 값은 shadow 비교용이며 active 시간 원천이 아니다.
- terminal의 `data.duration_spans[]`는 각 process source ID와 `duration_ms`를 가지며 terminal
  `duration_ms`는 span 합과 같아야 한다. 같은 process source ID 중복은 거부한다.
- 어느 경로에서도 monotonic span을 확보할 수 없으면 `duration_unknown_reason`을 기록한다.
...
시드 원본 `opal/core/setting.default.json`과 `scripts/install-mac.sh`의 `SEED_KEYS` 배선을 함께
검증하며, 기존 `~/.opal/setting.json`에 `timeZone`이 없으면 파일 rewrite 없이 fallback을 적용한다.
...
- 두 순번은 task transaction lock 안에서 `run/.runtime/run-log-{run_id}.sqlite3`의 `last_sequence`와
  `actor_cursors`를 확인한 뒤 발급한다.
...
SQLite의 cursor·request dedupe·capability table은 증거 SSOT가 아니라 runtime 파생 index다. 같은
락 안에서 JSONL append·fsync 후 transaction으로 갱신하며, 부재·손상·불일치 시 모든 segment의
순번·request ID·`worker.capability.issued/revoked` 사건을 1회 스캔해 전부 재구축한 뒤 append한다.
재구축 중에는 append와 worker 완료 mark를 fail-closed로 대기시킨다.
...
- `pending_events`는 사건당 UTF-8 4 KiB 이하의 compact 표준 payload를 `TOTAL_LIMIT=128`건까지
  보관하는 bounded outbox다. 일반 admission 한도는 `TOTAL_LIMIT - override 사건 수`로 계산한다.
  override는 `activity(kind=decision)`와 강제 `state.changed` 2건을 한 bundle로 정확히 한 번만
  일반 admission 한도에서 제외하며, 별도 예약 슬롯 자료구조는 두지 않는다.
...
- `completion_profile`은 `observed_trajectory | observed_terminal | cooperative`다. active는 앞의
  두 profile만 허용하며 설치된 adapter의 기계적 self-test receipt로 확정한다. `state-tool init`은
  adapter·receipt hash와 profile을 검증해 `completion_profile_receipt`에 고정하고 실패하면 active
  init을 거부한다.
...
8. 일반 사건이 `TOTAL_LIMIT - override 사건 수`에 도달하면 추가 상태 전이는 차단하고 reconcile
   또는 한 번의 사용자 override bundle만 허용한다.
9. `restart-run`은 새 `active_run_id`와 old `run.completed`·new `run.started` 두 outbox 사건을 한 번의
   state 원자 쓰기로 커밋한다. 각 사건이 가진 run ID로 재전송하며 둘 다 반영되기 전에는 pending이다.
...
- override는 승인된 상태 변경, `activity(kind=decision)`, 강제 `state.changed`를 같은 원자 쓰기로
  커밋하고 두 사건을 일반 admission 한도와 무관하게 outbox에 한 번만 기록한다.
...
| `run_log_outbox_full` | 일반 outbox가 `TOTAL_LIMIT - override 사건 수`에 도달 | reconcile·한 번의 사용자 override bundle 외 상태 전이 차단 |
...
worker log token verifier는 `worker.capability.issued` 사건에 원문이 아닌 SHA-256 hash·scope·만료만
기록하고 SQLite capability table에 투영한다. 비교는 constant-time으로 수행한다. terminal과
`run.completed(reason=restart)`는 그 자체로 해당 scope의 즉시 폐기 원천이며 별도 mirror 사건을
요구하지 않는다. terminal 전 조기 폐기만 `worker.capability.revoked`를 append한다. 재구축은 issued,
terminal, run.completed, revoked, expiry를 함께 적용하고 만료·폐기 token의 재사용을 거부한다.
`begin-worker`는 capability 발급 사건과 runtime 투영만 만들며 `worker.started`를 대신 만들지 않는다.
실제 `worker.started`는 adapter의 process start 또는 agent handshake 관측에서만 생성한다.
...
- active terminal의 `duration_source`는 `adapter_monotonic`이어야 하며, shadow에서는
  `worker_monotonic | unknown`도 비교 표본으로 허용한다.
...
- `tasks/**/run/.runtime/`은 cursor index와 capability verifier를 담으므로 `.gitignore`에 등록한다.
...
개정 기준일의 `tasks/**` 실측 기준선은 전체 약 20 MiB, AGENTIC-LOG 112개 합계 1,075,861
bytes(파일당 평균 약 9.6 KiB)다. 이 값은 run-log 예상량이 아니라 shadow 증가율을 계산할 비교
기준이며, backup을 포함한 동일 범위를 후속 측정에서도 유지한다.

Phase 1A shadow는 태스크별·worker별 표준 로그 bytes와 사건 수의 평균·p95·최대, 기존
AGENTIC-LOG 대비 증가율, git pack 증가량을 보고한다. 이 실측 없이 Phase 1B 보존 정책을 승인하지
않으며 4 MiB는 예상 사용량이 아니라 segment roll 안전 상한으로만 취급한다.
...
run-log-tool append             --task <task-path> --run-id <id> --request-id <id> --event <type> ...
...
`begin-worker`는 `worker_run_id`와 1회 디스패치 범위의 worker log token을 발급하되 started
사건은 만들지 않는다. token 원문은 worker 실행 컨텍스트에만 전달하고 로그에는 token ID만 남긴다. `restart-run`은 기존 run을
`run.completed(reason=restart)`로 닫은 뒤 새 run ID를 발급하며, 호출 전에 존재한 선행 미해소
outbox가 있으면 거부한다. restart 자체가 원자 커밋하는 두 사건은 이 사전검사의 대상이 아니다.
...
- import 멱등성 검증
- direct·adapter append의 `request_id` 멱등성 검증
- worker capability scope·만료·폐기 검증
- 디스크 기록 전 redaction
- run별 집계
...
| 실증 결과 | 배정 profile | 후속 |
|---|---|---|
| start·중간 사건·terminal 기계 관측 | `observed_trajectory` | 강한 궤적 게이트 후보 |
| start·terminal만 기계 관측 | `observed_terminal` | 약한 lifecycle 게이트 후보 |
| 기계 관측 원본 없음 | `cooperative` | shadow 유지, active 제외 |

스파이크 결과 없이 opds adapter 구현 가능성을 가정하거나 Phase 1A를 승인하지 않는다. 약한 profile은
...
5. `opds`(일반 Agent 채널)와 `oppl`(외부 Agent 호출 + 내부 CLI/raw 채널)의 profile별 adapter.
   Phase 0에서 cooperative로 판정된 경로는 변환기를 만들지 않고 shadow 대조군으로 남긴다.
...
채널별 승격 표본은 정상 완료 worker run 20건 이상과 즉시 실패·blocked·로그 쓰기 실패·동시 roll
각 1건 이상이다. `observed_trajectory`는 trusted started/activity/terminal, `observed_terminal`은
trusted started/terminal 포착률 100%, 예상 false-block 0건, 로그 누락 때문에 필요한 override 예상
0건, 실패 후 reconcile 성공률 100%를 모두 충족해야 한다. worker 자발적 direct activity 기록률과
누락률도 보고하지만 승격 판정에는 쓰지 않는다. `cooperative`는 수치와 무관하게 active 대상이 아니다.

### Phase 1B — 승격 적격 pilot enforcement

1. `opds`·`oppl`의 각 호출 경로 중 Phase 0 profile self-test와 Phase 1A 정량 기준을 통과한 경로만
   `run_log.mode=active`로 승격한다. 한 채널이 부적격이면 shadow를 유지하고 다른 pilot의 승격을 막지 않는다.
2. `observed_trajectory` 강한 게이트와 `observed_terminal` 약한 게이트, `gate.requested/resolved`
...
6. 승격 pilot의 active conformance, shadow 유지 pilot의 비차단 회귀, 기존 state/Console 회귀 검증

### Phase 1C — 나머지 pilot 확산

- 구조 확산 대상 8개 pilot은 다음 4배치로 adapter·SKILL·pipeline 계약을 확산한다:
  `opd+opdw`, `opp+opwt`, `opgc+oppd`, `opsdd+opdd`.
- 각 배치는 자기 pilot conformance와 전체 state 회귀를 통과한 뒤 다음 배치로 진행한다.
- 마지막 배치는 pilot 문서와 별도 소유 단위로 `task-process.md`, `parallel-execution.md`,
  `opal-harness-agentic.md`, `opal-harness-semi-agentic.md`, `docs/CONVENTIONS.md`, state-tool 테스트를
  함께 개정한다. 모든 pilot SKILL의 AGENTIC 무조건 기록 지시를 제거하고 기존 1.0/1.1 태스크의
  계속 실행과 import를 위한 legacy 조건만 남긴다.
- R-11은 모든 pilot 호출 경로가 Phase 0에서 실증된 profile을 선언하고 그 profile의 conformance를
  통과하면 완료다. cooperative 경로는 shadow 비차단 conformance로 판정하며 강한 궤적 달성률과
  분리 보고한다.
...
| R-2 | 지원 채널에서 워커의 의미 있는 궤적을 남긴다 | Phase 0 profile, 1A provenance, 1B gate | observed_trajectory만 adapter가 앞서 관측한 별도 activity source로 강한 궤적 달성 판정 |
...
| R-5 | 저장 순서와 인과 순서를 혼동하지 않는다 | runtime cursor index + actor_sequence + caused_by | index 재구축과 병렬 fixture가 순번을 보존하고 허위 선후관계를 만들지 않음 |
| R-6 | 상태 로그 장애가 state 전체를 교착시키거나 이력을 잃지 않는다 | 1A compact outbox, 1B override | 사건당 4 KiB·총 512 KiB 상한과 다중 누락 전이 멱등 복구 |
| R-7 | 워커 시간 원천이 하나다 | adapter monotonic span + duration projection | resume span 합산·ms→분 변환 및 불일치 진단 |
...
| R-10 | 기존 state 동작이 회귀하지 않는다 | additive schema/adapter | 기존 state-tool 전체 테스트와 `test_stats.py`·`test_config.py`·`test_routers.py`의 동결 fixture 회귀 통과 |
| R-11 | 표준 pipeline 전부가 실증된 profile 계약을 지킨다 | Phase 0, 1B 적격 pilot, 1C 8종 구조 확산 | 10개 skill의 호출 경로별 profile 선언과 active 또는 shadow conformance 통과 |
...
| R-18 | 두 시간계를 일관되게 해석한다 | UTC event + Asia/Seoul legacy/quietHours | `STATS-BASELINE.md`와 TS-130~137을 기준으로 기존 Asia/Seoul 결과를 보존하고 신규 timeZone 날짜 경계 fixture 통과 |
...
- Console 회귀: `tasks/103-260825-opd-태스크-진행통계/STATS-BASELINE.md`의 독립 기준값과
  `dashboard/backend/tests/fixtures/t103_states/` 동결 fixture를 소비하는 stats/config/router 테스트
- pipeline conformance: pilot별 통합 테스트
- 플랫폼 변환: 각 adapter가 소유한 canonical event fixture
- 최종 판정: `TEST-SCENARIO.md`의 R-1~R-21 매핑과 독립 test-agent 실행 결과
...
- [ ] Phase 0가 Agent 도구 외부 호출과 opal-agent CLI 내부 축을 각각 실호출해 completion profile을 판정한다.
- [ ] observed_trajectory 정상 완료는 trusted started + activity + terminal, observed_terminal은 trusted started + terminal + 기존 검증 계약을 통과한다.
- [ ] PM 대필·worker direct·import activity를 강한 궤적 기여에서 제외하고 검증된 adapter source만 observed_trajectory 증거로 인정한다.
...
- [ ] observed_trajectory는 activity 없는 정상 완료를 차단하고 adapter 기계 증거가 있는 `pre_activity_failure`만 예외로 허용한다.
...
- [ ] runtime cursor index 부재·손상·불일치 시 segment에서 재구축하고 동일 순번을 이어간다.
...
- [ ] active duration은 adapter monotonic process span 합이며 동일 worker resume의 다중 span도 중복 없이 합산된다.
...
- [ ] 마스킹된 run-log segment는 추적되고 `tasks/**/run/raw/`과 `tasks/**/run/.runtime/`만 기본 git 추적 대상에서 제외된다.
...
- [ ] Console 회귀는 STATS-BASELINE.md와 stats/config/router 동결 fixture의 실제 경로·명령으로 판정한다.
- [ ] provider별 원본 fixture가 동일한 canonical schema로 변환된다.
- [ ] 동일 request ID·payload 재시도는 기존 event를 반환하고, 다른 payload 재사용은 거부된다.
- [ ] worker token 원문은 남지 않고 issued/revoked 사건에서 runtime registry 전량을 재구축할 수 있으며 scope·만료·terminal/restart 폐기가 집행된다.
- [ ] `cooperative` 경로는 active 설정이 거부되고 shadow 비차단 conformance와 강한 궤적 미달성이 분리 보고된다.
- [ ] shadow 보고서가 태스크별·worker별 bytes/사건 수 평균·p95·최대와 git pack 증가량을 포함한다.
...
worker dispatch
    ├── observed_trajectory: trusted started → activity 1건 이상 → trusted terminal
    ├── observed_terminal:   trusted started → trusted terminal → 기존 artifact/PM 검증
    └── cooperative:         shadow 기록만, 완료 차단 불기여
