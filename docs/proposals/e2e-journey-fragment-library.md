...
> 상태: 검토 | 작성: 알투(PM) | 작성일: 2026-09-15 | 최종 갱신: 2026-09-18(태스크 141 — 검토 지적 5건 반영: 선행 조건 요약·신선도 키 driver/충실도·증적 마스킹 지점·승격 판정 주체·Q-5 결번 이력)
...
| 축 | 내용 | 착수 선행 조건 |
|---|---|---|
| 발동 | **`opal-e2e`** operator 스킬(alias `//e2e`) — 단계 파이프라인·워커 디스패치 없음. `//opbr`·`//oppm`·`//opas`와 같은 유형 | 없음 |
| 모드 | `author`(대화로 작성) · `import`(TEST-SCENARIO.md 변환) · `run`(실행) · `status`(조회) | 없음 |
| 저장 | 3폴더 — **추적 여부가 폴더로 갈린다**(§3) | 없음 |
| 재사용 | **조각(fragment)** — 이름 붙은 step 시퀀스 + 파라미터 + **사후 조건** | Q-1(중복 판정 충돌) + 증적 마스킹 보강(§4) |
| 범위 축소 | **신선도 키** — `(여정 해시, 조각 해시, surface_id, 대상 commit, 선택 driver 정체(driver+session_mode), 달성 충실도)` | 없음 |
| 확장 | **선언형 driver wrapper** — 새 브라우저는 JSON 한 장(§7). ADD-1이 이 절의 전제를 실측으로 확인했고 **ops 게이트(Q-6)를 선행 조건으로 드러냈다** | Q-6(ops 게이트) + driver 적합성 스위트(§7) |
| 협동 | 기존 `collaborative` profile·`awaiting_human`(exit 20)·resume token 재사용. 신규 구현 없음 | 없음 |
...
| 경우 | 경로 |
|---|---|
| 태스크 수행 중 | `(태스크)/e2e/`에 작성·실행. 기존 조각을 참고해 재사용한다. **CLOSE에서 실제로 통과한 것만** `docs/e2e/`로 승격하며, 이때 조각화 검토(같은 step 시퀀스가 두 여정에 반복되면 `fragments/`로 추출)를 함께 한다. **[MUST] 승격 자격은 `test-tool`의 exit 계약이 소유한 `pass` 증적 존재를 도구가 확인한 뒤에만 성립한다.** 사람은 조각화 검토(같은 step 시퀀스의 `fragments/` 추출 여부)만 판단하며, 사람의 산문 판단만으로는 `docs/e2e/`로 승격할 수 없다 — §6:128이 이미 세운 "판정은 스킬이 하지 않는다 … 판정은 `test-tool`의 exit 계약(0/6/7/18/19/20)이 소유" 경계를 승격 지점까지 잇는 것이다 |
...
- **[MUST] 자격증명을 조각에 직접 적지 않는다.** `value_ref`로 환경변수·시크릿 파일만 참조한다. 근거: 태스크 127 실측 — `fill`의 value는 C-6 redaction 대상이 **아니어서** `actions.jsonl`에 원문으로 남는다. **이 구멍을 막는 것이 조각 도입의 선행 조건이다.**
  - **적용 범위 한계**: `value_ref`는 조각 안 step만 덮는다. 조각 밖 본문에 직접 쓴 `fill`은 여전히 원문이 증적에 남는다.
  - **고쳐야 할 지점**: `opal/tools/test-tool/lib/e2e/redaction.py`의 `SECRET_HEADER_NAMES`(`:26`)·`SECRET_QUERY_KEYS`(`:39`)는 **키 이름 집합**이고, `redact_value()`(`:175`)의 dict 분기(`:196`)가 그 집합에 든 키만 `MASK`로 바꾼다. `{"kind": "fill", "target": "#pw", "value": "..."}` 레코드는 키가 `value`이므로 어느 집합에도 들지 않아 마스킹되지 않는다 — 이 판정 규칙을 보강해야 한다는 요구를 여기에 남긴다. 코드 수정은 이 제안의 범위가 아니다.
...
구성: `(여정 해시, 참조 조각 해시 집합, surface_id, 대상 commit, 선택 driver 정체(driver+session_mode), 달성 충실도)`. **휘발성 값(`run_id`·타임스탬프·포트)은 넣지 않는다** — 넣으면 영원히 일치하지 않아 무용지물이다.

재실행 생략은 아래 5개를 **모두** 만족할 때만 허용한다.
1. 대상 commit 동일  2. 여정·조각 해시 동일  3. 이전 결과가 `pass`  4. 증적이 실제로 남아 있음  5. 이전 증적의 driver 정체가 이번 실행에서 선택될 정체와 같고, 달성 충실도가 이번 요구 충실도 이상일 것

`.opal/e2e/order.json`(§7)으로 후보 순서를 바꾸면 선택되는 driver 정체가 달라지므로 키가 불일치해 **이전 `pass` 증적을 재인용할 수 없다.** 이 방어가 없으면 부분 driver가 1순위가 된 뒤에도 구 증적이 재인용된다.

충실도 비교는 "달성 충실도 ≥ 요구 충실도"로 판정하며 `mock`/`real-http`/`real-usage` 등급 정의를 여기서 복제하지 않는다 — `FIDELITY_ORDER`(`opal/tools/test-tool/lib/scenario.py:119`)가 단독 소유이며 이 절은 참조만 한다(§8).

**주의**: `api` profile은 상한이 `real-http`이므로 `real-usage`를 요구 충실도로 시드하면 키가 영원히 충족되지 않는다.

**한계**: driver 선택은 실행 환경(바이너리 설치 여부·`probe` 결과)에 좌우되므로 같은 commit에서도 선택 정체가 달라질 수 있다. 그러면 신선도 키가 자주 불일치해 재실행 생략이 사실상 성립하지 않고, §1이 지목한 "매번 전수 재실행" 문제로 되돌아갈 수 있다. 이 한계의 해소책 설계는 이 제안의 범위가 아니며 채택 시 후속 과제가 소유한다.
...
| # | 쟁점 | 메모 |
|---|------|------|
| Q-1 | **[착수 선행 조건]** 조각 전개분과 본문 연산의 **중복 판정 충돌** | 동결 RED S-27 (d-1)이 같은 연산 signature 중복을 재시도로 판정한다. 태스크 127에서 `--version` 2회가 실제로 이 규칙에 걸려 browser step runner가 봉쇄됐다가 driver 인스턴스 메모이즈로 해소됐다. 조각이 `navigate`를 포함하고 본문도 쓰면 재발한다 — **전개분과 본문을 구분 집계**하거나 중복 판정 범위를 좁혀야 한다. **승격 근거**: §1 표 1행(`:15`)이 "`test-scenario.json`은 `scenario-lock` 이후 spec존 변경이 불가"를 실측 사실로 적고 있어, 동결 RED S-27이 사후 변경 불가라는 이 제약이 곧 "조각 전개가 동결 RED S-27에 걸리면 조각 기능 자체가 구현 불가"라는 인과로 이어진다 — 착수 여부를 가르는 항목이므로 Q-6과 같은 등급에 둔다 |
...
| Q-5 | ~~`import` 모드의 한계~~ → **결번(번호 이관, 내용 소실 없음)** | 커밋 `6d1c8f1`까지 Q-5는 "`import` 모드의 한계"였다. 커밋 `28e91e1`(ADD-1 반영)에서 Q-4가 Ego Lite로 재배정되며 구 Q-4(`.e2e/` 이름 충돌)·구 Q-5가 각각 Q-8·Q-9로 이관되어 Q-5 번호만 비었다. 현 위치는 Q-9 |
