...
- 커밋 실행 시점 규칙(기본 사용자 승인 · 등록된 전용 worktree의 모드별 체크포인트 예외)은 `opal/core/references/harness/guards.md` §커밋 규칙이 소유한다 — 본 절은 커밋 **메시지 형식·단위**만 규정한다.
- 커밋 메시지는 한국어
- 하나의 태스크 = 하나의 merge 단위 (원칙). 등록된 전용 worktree에서는 `guards.md`가 허용한 안정 경계별 체크포인트 커밋을 복수로 만들 수 있으며, 사용자 승인 merge가 이 커밋들을 하나의 태스크 단위로 기본 브랜치에 반영한다.
- CLOSE 시 메모리 히스토리 행은 `state-tool mark`가 자동 생성한다(088). PM은 `result` 필드를 보강해 최종 체크포인트 또는 finalize 커밋에 포함하며, 히스토리 갱신용 별도 커밋을 만들지 않는다.
...
- **전체 목록: `opal/tools/` (25종)** — 위 6종 외 `event-loader`, `xlsx-tool`, `skill-registry`, `playwright-tool`(opt-in — 기본 설치에서 제외, 127), `ego-browser-tool`, `improve-tool`, `cmux-tool`, `git-sync-tool`, `worktree-tool`, `date`, `doctor`, `tool-scan`, `opal-cli`, `opal-agent`, `opal-action-monitor`, `run-log-tool`, `self-pm-tool`, `oppl-runtime-tool`, `oppb-runtime-tool`.
