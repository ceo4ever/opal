...
- **전체 목록: `opal/tools/` (22종)** — 위 6종 외 `event-loader`, `xlsx-tool`, `skill-registry`, `playwright-tool`(opt-in — 기본 설치에서 제외, 127), `improve-tool`, `cmux-tool`, `git-sync-tool`, `worktree-tool`, `date`, `doctor`, `tool-scan`, `opal-cli`, `opal-agent`, `opal-action-monitor`, `self-pm-tool`, `oppl-runtime-tool`.
