...
- 워크트리(`{프로젝트}/.opal-worktrees/task_{NNN}/`)에서 **허브 고정 데이터**(`tasks/`·`.opal/`)를 참조하는 경로 판정 규칙의 원문 SSOT는 `opal/core/references/opal-harness.md` **§2.5 (4)**다. 본 문서는 포인터만 둔다.
- **적용/제외 경계**: 정규화는 **허브 고정 데이터 참조**에만 적용하고 **소스 트리 내용**을 다루는 경로(브랜치 소스 검증·빌드·배포 대상, 진단 도구)에는 적용하지 않는다 — 적용하면 워크트리가 자기 변경 대신 허브 소스를 보게 되어 검증의 의미가 뒤집힌다.
- 프로젝트 루트 판정은 **자기 `.opal/`을 가진 첫 조상이 이긴다**. 세그먼트 정규화는 그 탐색이 실패했을 때의 해석이며 탐색을 대체하지 않는다(§2.5 (4) 부칙 「탐색 우선순위」).
- 구현·테스트의 실행 가능한 대조 기준은 골든 케이스 표 `opal/core/references/hub-root-cases.json`(C-1~C-7)이다. 런타임별 구현은 규칙 원문을 재서술하지 않고 §2.5 (4) 포인터 주석만 둔다.
...
| v1.10.0 | 2026-09-07 17:14 | §구현 규칙에 `허브 루트 해석` 항목 신설 — 워크트리에서 허브 고정 데이터(`tasks/`·`.opal/`)를 참조하는 경로 판정의 원문 SSOT가 `opal/core/references/opal-harness.md` §2.5 (4)임을 포인터로 명시하고, 적용/제외 경계(소스 트리·진단 도구 제외)·탐색 우선순위(자기 `.opal/`을 가진 첫 조상이 이긴다)·골든 케이스 표(`hub-root-cases.json` C-1~C-7)를 함께 기재한다. 규칙 원문은 복제하지 않는다 (109) |
