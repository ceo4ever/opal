...
3. **[MUST]** `~/.opal/references/opal-harness.md`를 Read한다. 이 단계를 건너뛰면 §1 구현 금지 원칙 및 게이트 프로세스를 인지하지 못한 채 PM 역할이 활성화될 수 있다.
4. 스킬 레지스트리 도구 확인:
...
5. 참조 문서를 Read로 읽어 공통 규칙을 인지한다:
   - `~/.opal/references/agents.md` — 에이전트 목록 및 호출 규칙
   - `~/.opal/references/mcps.md` — MCP 서버 목록
6. 모델 매핑 로드: `~/.opal/references/opal-model-mapping.md`를 Read하고, 플랫폼을 감지하여 매핑을 적용한다 (아래 "모델 매핑 자동 적용" 참조).
7. 현재 프로젝트에 OPAL 부트스트래퍼가 없으면 자동 삽입한다 (아래 "프로젝트 부트스트래퍼 자동 관리" 참조).
8. PM 컨텍스트 로드: `{프로젝트}/.opal/AGENT.md`가 존재하면 Read하여 PM 역할을 활성화한다 (아래 "PM 컨텍스트 로드" 참조).
9. 프로젝트 메모리 브리핑: `{프로젝트}/.opal/MEMORY.md`가 존재하면 읽고, 최신 메모리를 소유자에게 브리핑한다 (아래 "프로젝트 메모리 브리핑" 참조).
10. 정체성 + 레지스트리 + PM 로드 후, 아래 규칙에 따라 에이전트로 활성화한다.
...
- 수행 완료: ✅  |  미수행/실패: ❌  |  해당 없음(PM 없는 프로젝트 등): ⬜
...
부트스트랩 8단계에서, `.opal/AGENT.md`가 존재하면 PM 역할을 활성화한다.

> **전제 조건**: `opal-harness.md`(부트스트랩 3단계)가 로드되어 있어야 한다. 미로드 상태에서 PM 역할을 활성화하지 않는다.
...
부트스트랩 9단계에서, 프로젝트 메모리가 존재하면 소유자에게 최신 내용을 브리핑한다.
...
부트스트랩 6단계에서, 모델 매핑을 로드하고 현재 플랫폼에 맞게 적용한다.
...
부트스트랩 7단계에서, 현재 프로젝트에 OPAL 부트스트래퍼가 없으면 자동으로 삽입한다.
