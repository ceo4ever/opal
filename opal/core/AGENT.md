...
| ⑤ | `## ▶️ 승인 요청` | 무엇을 왜 결정해야 하는지 서술(권고·영향 포함) + 승인 채널 2형태 중 하나(아래 원칙 7) | **다음 액션이 사용자 입력 없이는 실행 불가할 때 — 필수** |
...
| 2 | **지금 사용자 답이 필요한가?** — 「② 다음 액션」을 사용자 입력 없이 실행할 수 있는가로 판정한다(없으면 `▶️`) | `▶️` | `📌` |
...
- <권고안 1문장 — 왜 이것인지 근거 포함>
- <이 결정이 무엇에 영향을 주는지 1문장>

이대로 진행할까요?
```
위는 **(A) 확인형** — 권고안이 하나로 설 때. `AskUserQuestion`을 호출하지 않고 마무리 문장으로 받는다.
...
- <이 결정이 무엇에 영향을 주는지 1문장>
```
위는 **(B) 선택형** — 후보가 이산적·배타적이고 어느 쪽인지 답이 서지 않을 때. **블록 직후 `AskUserQuestion`을 1문항으로 호출**한다.
...
7. **[MUST] 승인 요청은 한 번에 한 사안이며 채널은 2형태다** — `▶️` 블록에 결정 사안·권고·영향을 자세히 쓰고, 아래 둘 중 하나로 받는다. 결정 사안이 2개 이상이면 가장 앞선 하나만 묻고 나머지는 그 답 이후로 미룬다. **자동 진행 금지**(승인 없이 다음 단계로 넘기지 않는다)와 **교착 금지**(묻지도 진행도 하지 않는다)가 함께 적용된다.
   - **(A) 확인형** — 분석으로 권고안이 하나로 설 때. 권고 1개 + 근거를 쓰고 「이대로 진행할까요?」로 받는다. `AskUserQuestion`을 호출하지 않는다.
   - **(B) 선택형** — 후보가 이산적·배타적이고 어느 쪽인지 답이 서지 않을 때. 직후 `AskUserQuestion`을 **`questions` 1개**로 호출하고, 권고안은 첫 선택지에 두고 라벨 끝에 `(권고)`를 붙인다.
...
| 10 | `- [ ]` 체크박스로 승인 요청 | 승인 채널은 `▶️` 블록 하나뿐 — 그 안에서 확인형·선택형으로 갈린다(원칙 7) |
...
- 승인 요청이 한 사안이고 확인형·선택형 중 맞는 채널로 받았는가.
