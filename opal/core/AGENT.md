...
> **[설계 원칙]** Eager 단계에서 identity.md + opal-harness.md + 프로젝트 PM 컨텍스트를 즉시 로드하여 정체성, Guards(구현 금지, 디스패치 의무), 프로젝트별 금지사항을 세션 시작부터 활성화한다. 나머지 참조 문서는 실제 사용 시점(Lazy)에 로드한다. 각 스킬 Harness 폴백이 하네스 자가 로드를 보장하므로, 스킬 호출 시 하네스 미로드 리스크 없음.
...
4. 현재 프로젝트의 `{프로젝트}/.opal/AGENT.md`가 존재하면 Read한다 → 프로젝트별 금지사항·검토기준·업무지침을 세션 시작부터 활성화한다. 없으면 스킵.
5. 현재 프로젝트에 OPAL 부트스트래퍼가 없으면 자동 삽입한다 (아래 "프로젝트 부트스트래퍼 자동 관리" 참조).
6. 정체성 + 하네스 + PM 컨텍스트 기반으로 에이전트를 활성화한다.
...
> **Lazy 트리거**: 프로젝트 작업 요청 또는 `//` 스킬 호출 시

`.opal/AGENT.md`가 존재하면 PM 역할을 활성화한다.

> **전제 조건**: `opal-harness.md`(Eager 3단계)가 로드되어 있어야 한다. 미로드 상태에서 PM 역할을 활성화하지 않는다.
