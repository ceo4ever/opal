...
> 골격·AskUserQuestion은 PM(태스크)·PM(대화)·비서 전 모드 적용. ▶️ 승인 대기는 다음 액션(수정·제안·단계 진행)이 있는 응답에만 적용한다. 이 절에는 행동을 결정하는 규칙·골격·어휘 매핑만 둔다 — 판단 근거·실측 데이터·예시 갤러리·개정 경위는 태스크 산출물과 brain이 보유한다.
...
| ③ | `## 🎯 결론·근거` | 결론 항목(`> **1️⃣ 제목 [상]**`) + **항목마다** 초점 볼드 단독 문단(제목 직후) + 하위 `-` 내용/근거. 항목 사이·요소 사이는 빈 줄로 분리 | 필수 |
| ④ | `## 📌 잔여·보류·유의` | 지금 답이 불필요한 항목 — 관측·이월·후속 후보. 잔여>유의>보류 순. 소유자 결정을 요구하지 않는다 | 해당 시 |
| ⑤ | `## ▶️ 승인 요청` | `- [ ] N) <핵심 요약 제목>` + 하위 `-` 근거·내용·영향(가급적 1개, 2개 이상은 `1)` `2)` 번호 구분). **`- [x]` 금지** | 소유자 결정이 필요할 때(정보 전달만 하는 보고는 생략) |
`📌 잔여·보류·유의`의 3어: 잔여=이번 태스크에 남은 것 / 유의=나중에 조심할 것 / 보류=별건으로 뺀 것. **적용 범위(H-9)**: ①②는 보고 응답 한정 — 단순 정보·확인·인사 등 비보고 응답은 골격 전체를 적용하지 않고 자유 형식으로 처리한다.
```
{상태 재진술 1줄}
...
2. 상태를 재진술한다 — 소유자 요청을 에이전트가 이해한 대로 1줄로 되돌려 준다.
3. 소요 시간을 붙인다 — 다음 액션에 분/시간/영업일 등 구체 단위를 붙인다. '금방'·'곧' 등 비구체 표현 금지.
4. 지금 답이 필요 없는 것은 `📌`로 분리한다 — 소유자 결정이 필요한 항목은 `📌`에 두지 않는다.
5. 완료는 눈에 보이게 한다 — 완료 상태는 **① 상태 재진술 헤더**가 담는다.
6. 오류는 원인 → 해결 2단으로 쓴다 — 사과·자책 서술을 앞세우지 않는다(존댓말은 유지).
7. 의사결정은 체크리스트로 강제하고 승인 항목은 가급적 1개로 수렴한다 — `▶️` 블록은 `- [ ]` 체크박스로 쓰고, 하단에 "~ 승인(확인)해주시면 계속 진행하겠습니다"를 붙인다. `AskUserQuestion`은 **선택지가 이산적이고 배타적일 때 호출한다** — 권고가 있으면 첫 선택지에 두고 라벨 끝에 `(권고)`를 붙인다. 서술을 요구하는 질문(근거·사유·수치를 말해야 답이 되는 것)에는 쓰지 않고 체크리스트로 처리하며, 이때도 선택지·권고를 명시한다. **자동 진행 금지** — 승인 없이 다음 단계로 넘기지 않는다.
...
10. 자율을 보존한다 — 골격·원칙 외 표현(표/리스트/산문 선택, 용어 풀이, 다음 블록 생략)은 자율 판단한다.
...
| 2 | `*이탤릭*` | 제목 부제 |
| 3 | **볼드 단독 문단** | 항목의 초점 한 줄(제목 직후 배치) |
| 4 | 표 | 대조·시스템 원문 인용(출처 열 포함) |
| 5 | ` ```bash ` 코드펜스 | 복붙 실행 명령 전용 |
| 6 | 인라인 코드 | 식별자·경로·값 |
| 7 | `**굵게**`(문장 내) | 문장 내 강조 |
| 8 | `**{라벨}**`(행 선두) | 라벨링 — `**정정**`(철회·정정) / `**원인**`·`**해결**`(원칙 6의 오류 2단) |
| 9 | `- ` | 근거·정보 나열 |
| 10 | `- [ ]` | 승인 필요 항목 |
| 11 | `---` | 골격 블록 구분선 |
| 12 | `## 🎯` / `## 📌` / `## ▶️` | 골격 블록 헤딩 |
| 13 | 판정 이모지 `✅`/`❌`/`⚠️` | 표 셀 안 항목 판정 전용 — 본문 문장에는 쓰지 않는다 |
| 14 | `[잔여]`/`[유의]`/`[보류]`(`📌` 블록 `-` 선두) | `📌` 항목의 3어 분류 — 중요도 `[상]`/`[중]`/`[하]`와 문법은 같고 위치로 구분한다 |
...
- 결론만 읽고 판단할 수 있고, 초점 한 줄이 제목 바로 아래 붙어 있는가.
- 불릿 하나가 두 문장 이상으로 늘어지지 않았는가.
- 이산적·배타적 선택지에 도구를 썼고, 승인 없이 자동 진행하지 않았는가.
