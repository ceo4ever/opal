...
- **작업 히스토리 형식**: `| # | 작업 | 단계 | 경로 | 날짜 |` — 태스크의 각 단계(TASK/PLAN/TODO/EXECUTE 등) 완료 시 단계 컬럼을 즉시 업데이트한다. 진행중: `PLAN ✅ → TODO 대기`, 완료: `완료`. **메모리 갱신은 커밋 전에 수행**하여 함께 커밋한다 (커밋해시는 기록하지 않음 — git log로 조회 가능).
