...
- **메모리 인덱스 형식**: `| # | 등록일시 | 카테고리 | 상태 | 파일 | 설명 |` — 메모리 등록 시 순번과 등록일시(`YYYY-MM-DD HH:mm`)를 기록한다.
- **작업 히스토리 형식**: `| # | 작업 | 단계 | 경로 | 시작일시 | 완료일시 |` — 태스크 시작 시 시작일시(`YYYY-MM-DD HH:mm`)를 기록하고, 각 단계 완료 시 단계 컬럼을 즉시 업데이트한다. 진행중: `PLAN ✅ → TODO 대기`, 완료: `완료` + 완료일시 기록. **메모리 갱신은 커밋 전에 수행**하여 함께 커밋한다.
