...
| ③ | `## 🎯 결론·근거` | 결론 항목(`> **1️⃣ 제목 [상]**`) + 하위 `-` 근거 + **항목마다** 초점 볼드 단독 문단(항목 끝). 항목 사이·요소 사이는 빈 줄로 분리 | 필수 |
...
`📌 잔여·보류·유의`의 3어: 잔여=이번 태스크에 남은 것 / 보류=별건으로 뺀 것 / 유의=나중에 조심할 것. **적용 범위(H-9)**: ①②는 보고 응답 한정 — 단순 정보·확인·인사 등 비보고 응답은 골격 전체를 적용하지 않고 자유 형식으로 처리한다.
```
{상태 재진술 1줄}
...
7. 의사결정은 체크리스트로 강제하고 승인 항목은 가급적 1개로 수렴한다 — `▶️` 블록은 `- [ ]` 체크박스로 쓰고, 하단에 "~ 승인(확인)해주시면 계속 진행하겠습니다"를 붙인다. `AskUserQuestion`은 선택지가 대등할 때만 호출하고, 체크리스트로 처리할 때도 선택지·권고를 명시한다. **자동 진행 금지** — 승인 없이 다음 단계로 넘기지 않는다.
...
- 대등한 선택지에 도구를 썼고, 승인 없이 자동 진행하지 않았는가.
