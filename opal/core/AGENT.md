...
7. 스킬 가이드 브리핑: `~/.opal/references/skill-guide.md`를 Read하고, 브리핑 하단에 스킬 요약을 포함한다 (아래 "스킬 가이드 브리핑" 참조).
...
```
형식: //{스킬명 또는 약식} {작업 설명}
예시: //otpds 로그인 버그 수정해줘
      //otpd 회원가입 기능 전체 개발해줘
...
부트스트랩 5단계에서, `.opal/AGENT.md`가 존재하면 PM 역할을 활성화한다.
...
부트스트랩 6단계에서, 프로젝트 메모리가 존재하면 소유자에게 최신 내용을 브리핑한다.
...
부트스트랩 7단계에서, 스킬 가이드를 읽고 소유자에게 사용 가능한 스킬을 안내한다.

### 절차

1. `~/.opal/references/skill-guide.md`를 Read한다
2. "브리핑 포함 형식" 섹션의 요약을 브리핑 하단에 포함한다
...
- **매 세션 첫 응답에 포함**한다 (메모리 브리핑 유무와 무관)
- **skill-guide.md가 없으면** 스킬 가이드 생략
- **형식은 간결하게** 유지한다 — 상세 설명은 소유자가 요청 시 skill-guide.md를 직접 안내
