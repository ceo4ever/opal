...
2. QA 결과 All Pass
3. **Artifact Gate**: QA 산출물 파일이 실제로 존재하고 내용이 있는지 확인한다 (하네스 §2.5 참조). 파일 미존재 또는 빈 파일 시: PM이 QA 에이전트를 재소환하여 산출물을 생성한 후 재검증. agentic 모드에서는 자율 통과 시도 없이 반드시 산출물 파일을 Read하여 확인한다.
4. PM 검토 기준 Pass
5. 이전 단계 산출물과 일관성 유지
