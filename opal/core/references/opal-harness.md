...
(3)은 태스크 문서(`tasks/`)·`.opal/`이 **어디에 있는지**(허브 고정)를 정한다. 이 항은 그 고정된 위치를 **어느 작업본에서 실행되든 찾아내는 판정**을 정한다 — (3)의 보완이며, 고정 대상을 바꾸지 않는다. 본 규칙의 **원문은 이 항 한 곳에만 존재한다**. 런타임 구현은 주석에 `opal/core/references/opal-harness.md §2.5 (4)`를 적어 이 항을 가리키고, 정의를 복제하지 않는다.
...
4. **적용 대상 경계** — 정규화는 **허브 고정 데이터**((3)의 `tasks/`·`.opal/`) 참조에만 적용한다. **소스 트리 내용**을 다루는 경로(브랜치 소스 검증·빌드·배포 대상)에는 적용하지 **않는다** — 적용하면 워크트리가 자기 변경 대신 허브 소스를 보게 되어 검증의 **의미가 뒤집힌다**.
...
- **탐색 우선순위** — **cwd가 워크트리 안일 때만**, 실행 지점에서 상향 탐색해 자기 `.opal/`을 가진 첫 디렉터리를 프로젝트 루트로 삼는다. 그 탐색의 **상한은 허브**이며 허브 위로 올라가지 않는다. 세그먼트 정규화는 이 탐색이 `.opal/`을 찾지 못했을 때의 해석이며 탐색을 대체하지 않는다. 두 근거가 이 순서를 요구한다. ① `.opal/`은 (3)에 의해 허브 고정이므로 **워크트리에서 상향 탐색하면 그 탐색 자체가 허브에 닿는다** — 정규화 없이도 목적이 달성된다. ② 자기 `.opal/`을 가진 **자기완결 프로젝트가 워크트리 경로 하위에 있을 수 있다**(테스트 픽스처가 그렇다). 그것을 허브로 덮어쓰면 자기 설정 대신 허브 설정으로 스캔돼 4항(소스 트리 미적용)을 위반한다.
