...
- **탐색 우선순위** — 프로젝트 루트를 정할 때는 **실행 지점에서 상향 탐색해 자기 `.opal/`을 가진 첫 디렉터리가 프로젝트 루트다.** 세그먼트 정규화는 그 탐색이 `.opal/`을 **찾지 못했을 때**의 해석이며, 탐색을 대체하지 않는다. 두 근거가 이 순서를 요구한다. ① `.opal/`은 (3)에 의해 허브 고정이므로 **워크트리에서 상향 탐색하면 그 탐색 자체가 허브에 닿는다** — 정규화 없이도 목적이 달성된다. ② 자기 `.opal/`을 가진 **자기완결 프로젝트가 워크트리 경로 하위에 있을 수 있다**(테스트 픽스처가 그렇다). 그것을 허브로 덮어쓰면 자기완결 프로젝트가 자기 설정 대신 허브 설정으로 스캔돼 4항(소스 트리 미적용)을 위반한다.
