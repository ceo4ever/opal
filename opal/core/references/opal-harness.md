...
> **[강제]** 아래 각 이벤트 발생 시 STATE.md 갱신은 **필수**다. 갱신 미수행 시 다음 단계 진입이 금지된다. State Gate(아래 §3 State Gate 섹션 참조)가 이를 확인하며, 미갱신 감지 시 차단한다.

| 이벤트 | 갱신 주체 | 내용 | 강제 여부 |
|--------|----------|------|----------|
| TASK 완료 | 오케스트레이터 | STATE.md 초기 생성 | **필수** |
| 단계 시작 | 오케스트레이터 | 단계, 상태: 진행 중 | **필수** |
| 단계 완료 | 오케스트레이터 | 완료 산출물 갱신, 상태: QA Gate 대기 | **필수** |
| QA Gate 통과 | PM | 상태: QA Gate 대기 → PM Gate 대기 | **필수** |
| PM Gate 통과 | PM | 상태: PM Gate 대기 → 사용자 확인 대기 | **필수** |
| 사용자 확인 완료 | PM | 상태: 사용자 확인 대기 → 완료 | **필수** |
| EXECUTE Step 완료 | 워커 (1차) + PM (확인) | 진행: Step N/M 완료 | **필수** |
| 블로커 | 워커 | 상태: 블로커 + 블로커 섹션 | **필수** |
| 완료 | 오케스트레이터 | 상태: 완료 | **필수** |
| 추가작업 진입 | 오케스트레이터 | 상태: 추가작업중 | **필수** |
| 추가작업 완료 | 오케스트레이터 | 상태: 추가작업완료 | **필수** |

**갱신 모델**: 워커가 1차 갱신을 수행하고(best effort), PM이 State Gate에서 확인하여 미갱신/오갱신 시 즉시 보완한다. QA 체크리스트와 동일한 2단계 구조.
...
> **레거시 호환**: 기존 STATE.md의 `대기 중` 값은 `QA Gate 대기`와 동등하게 처리한다. 소급 변경은 불필요하며, 신규 작성 시 새 값을 사용한다.
