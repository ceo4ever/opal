...
| 검증 결과 | 검증 통과 여부 (문서 변경: PM Gate 문서검증 / 코드 변경: 동작 검증 TEST·lint·build) |
...
| 스킬 | 추가작업 검증 | 비고 |
|------|-------------|------|
| opp | PM Gate 문서검증 | 문서 변경 검증 (요구사항→설계 검토를 PM이 직접 수행) |
| opds | lint + build + 관련 테스트 | 간략 검증 (동작 검증) |
| opd | 전체 테스트 스위트 + PM Gate 문서검증 | 풀 검증 (동작 검증 + 문서검증) |
| opwt | PM Gate 문서검증 | 기획 문서 검증 |
| opsdd | PM Gate 문서검증 | 설계 문서 검증 |
...
| v1.3 | 2026-06-07 | QA→PM Gate 통합 정합화 — 스킬별 검증 테이블 opp/opwt/opsdd의 "QA Gate (QA 에이전트)" → "PM Gate 문서검증", opd의 QA Gate → PM Gate 문서검증. ADD_DONE 검증 결과 필드를 문서검증/동작 검증으로 구분. 동작 검증(TEST/lint/build) 영역 불변 (014 Phase 4-2) |
