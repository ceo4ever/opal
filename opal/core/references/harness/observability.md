...
워커 디스패치에서는 매번 `event-loader`의 `worker.dispatch` load 응답으로 이 문서 전문을 적용하고 receipt를
검증한다. 과거 디스패치의 이 문서 내용이나 receipt를 재사용하지 않으며, 검증 전에는 아래
행위 주체 선언과 Agent 호출을 시작하지 않는다.
...
**트리거**: PM이 Agent 도구로 에이전트를 디스패치할 때. 아이콘 룩업·아래 선언 형식은 이 트리거(PM 발화 + Agent 도구)에만 적용된다.

Agent 도구로 에이전트를 디스패치할 때, 해당 에이전트 AGENT.md의 frontmatter `icon` 필드를 읽어 사용한다.
`icon` 필드가 없으면 디폴트 아이콘 `✨`을 사용한다.

**적용 범위 제외 — opal-agent 내부 채널**: 루프 액션 에이전트(`opal-loop-action-agent`) 등 서브에이전트가 opal-agent(headless CLI) 채널로 내부 디스패치하는 경우는 (a) PM 발화가 아니고 (b) Agent 도구 호출도 아니므로, 위 트리거에 해당하지 않아 아이콘 룩업·선언 형식의 대상이 아니다. 해당 채널의 관측성은 호출 주체(예: 루프 액션 에이전트)가 결과 파일(`.oppl-run/*` — `opal/agents/opal-loop-action-agent/AGENT.md` §결과 파일 규약 참조)과 결과 요약으로 자체 확보한다.
