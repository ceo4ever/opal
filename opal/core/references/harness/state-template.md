...
> **[MUST] STATE.md를 LLM이 직접 작성하는 것은 금지된다 — 반드시 `state init` 호출로 생성한다.** 기본값 `semi-agentic`(소유자가 `--interactive`/`--agentic` 명시 호출 시에만 전환).
>
> ```bash
...
> ```
>
> - `--task-title`: STATE.md 1행 제목(생략 시 `<task-path>` 마지막 디렉토리명). `--next-action`: `## 다음 액션` 초기값(생략 시 `"PLAN 단계 진입"`, TASK가 첫 단계면 `"TASK 단계 진행"`)
>
...
> <!-- pipeline:end -->
> ```
> 마커 손실 시 갱신 명령이 `marker_missing` 에러로 거부(`PLAN §2.18 #2`) — `show` 명령만 fallback 출력으로 우회.
> **자유 텍스트 3개 섹션 자동 생성 (§2.11 G-8)**: `state init` 실행 시 아래 3개 섹션을 자동 생성한다.
>
> | 섹션 | 초기 생성 내용 | 이후 갱신 |
> |------|-------------|----------|
> | `## 의사결정 로그` | 빈 표 (`| # \| 시점 \| 결정 \| 근거 \|` + 헤더 구분선) | 자동 추가 (§2.17) |
> | `## 블로커` | `없음` | PM 수동 갱신(state-tool 범위 밖) |
> | `## 다음 액션` | 자동 파생값(첫 줄) | `advance`/`mark` 시 프론티어(첫 미완료 행)에서 자동 파생·갱신(첫 줄만 치환, 하위 자유 기재 보존), `init`/전이 `--next-action` 오버라이드(비지속 — 다음 전이 미오버라이드 시 자동 파생 복귀, 072) |
>
...
## 다음 액션
{init 시 --next-action 인자 또는 기본값 "PLAN 단계 진입" — 이후 advance/mark 시 프론티어에서 자동 파생 갱신, 072}
```

**파이프라인 현황판 행 구성 규칙**: TASK 단계 = `작업`/`사용자 확인`(Gate 없음, TASK.md 생성은 `작업`에 흡수). 일반 단계(PLAN/EXECUTE 등) = `작업`/`PM Gate`/`사용자 확인` 순 — 산출물 생성, 문서 QA(PM Gate 흡수, `harness/pm-review-gate.md` §문서 QA 검증 참조), state 기록(행 mark 자체)은 모두 별도 행 없이 해당 행에 흡수한다(QA Gate/State Gate 행 없음, 단계 건너뛰기는 state-tool stage-transition guard 차단). CLOSE 단계(마지막 단계) = `DONE.md 생성` 1행(Gate·사용자 확인 없음 — 직전 단계 사용자 확인이 CLOSE 진입 게이트). Gate 없는 단계(opp TASK 등)는 PM Gate 행 생략. EXECUTE는 산출물 행 없음(`작업`에 흡수). 오케스트레이터 SKILL.md "STATE.md 도메인 치환값"에 스킬별 행 예시가 명시됨.

> **CLOSE 진입 게이트**: 사용자의 확인된 지시(`승인`/`확인`/`확인완료` 등)가 없으면 CLOSE 진입 불가(agentic 모드도 동일).

**산출물 행 규칙**: 산출물 생성은 `작업` 행에 흡수한다(별도 행 없음 — 워커가 산출물 생성 후 PM이 존재·내용 확인하여 ✅ 처리). DONE.md 행은 CLOSE 단계의 유일 행이다.

> **레거시 호환**: CLOSE 단계 도입 전 STATE.md는 소급 변경하지 않는다 — 신규 태스크부터 반영. 기존 "최종 단계 마감 블록"은 레거시 구조로 유효하다.
...
| v1.7 | 2026-08-09 21:07 | 중복 서술 압축 — 초기 블록·행 구성 규칙·산출물 행 규칙·레거시 호환 산문을 표/병합 축약. `state-tool` 명령 행 1건 보존. 114→99줄 (C1) (087) |
