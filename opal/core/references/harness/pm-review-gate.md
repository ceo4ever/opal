...
**[MUST] 소요를 먼저 적어 둔다.** 완료 알림에 실린 `duration_ms`를 분으로 환산(반올림)해 보관한 뒤, 해당 행을 `mark`할 때 `--worker-duration-minutes`로 전달한다(명령 원문은 `opal-harness.md` §3). **이 알림은 세션과 함께 사라지므로 지금 적지 않으면 값이 영구히 소실된다** — 행에는 완료 시각만 남고 워커 시작 시각이 없어 사후 계산이 불가능하다. 놓치면 그 시간은 통계에서 PM 몫으로 잘못 귀속된다.
...
| 버전 | 날짜 | 내용 |
|------|------|------|
| v1.10 | 2026-08-26 | §자가 진단 — 워커 기록 확인을 2항으로 분리(3번 `--as-worker` 마킹 여부 / 4번 소요 기록 또는 미측정 선언, 기존 4→5번 이동). 침묵이 CLOSE에서 차단됨을 명시 — 경고만으로는 무시하면 통과하므로 태스크를 닫는 지점에서 한 번은 반드시 걸리게 했다 (태스크 103) |
| v1.9 | 2026-08-26 | §워커 완료 선언에 [MUST] 「소요를 먼저 적어 둔다」 신설 — 완료 알림의 `duration_ms`를 분으로 환산해 보관 후 `mark --worker-duration-minutes`로 전달. §자가 진단에 확인 항목 3번 신설(기존 3→4번으로 이동). 이 알림은 세션 소멸과 함께 사라지므로 수신 직후가 유일한 기록 시점이다 (태스크 103) |
