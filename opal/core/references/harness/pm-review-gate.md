...
워커 결과 수신 직후, Observability 선언을 수행한다 (`harness/observability.md` 참조):

> `⚙️ 워커 완료:` {단계명} — {결과 한 줄 요약}

**[MUST] 소요를 먼저 적어 둔다.** 완료 알림에 실린 `duration_ms`를 분으로 환산(반올림)해 보관한 뒤, 해당 행을 `mark`할 때 `--worker-duration-minutes`로 전달한다(명령 원문은 `harness/state.md`). **이 알림은 세션과 함께 사라지므로 지금 적지 않으면 값이 영구히 소실된다** — 행에는 완료 시각만 남고 워커 시작 시각이 없어 사후 계산이 불가능하다. 놓치면 그 시간은 통계에서 PM 몫으로 잘못 귀속된다.
...
> 동일 컨텍스트 재개 횟수 상한은 `harness/guards.md` §자동 루핑 제약 표를, 재배치 분할은 `pm/dispatch-process.md` Step 1의 비중첩 변경 범위 계약을 따른다.
