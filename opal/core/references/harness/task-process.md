...
`op-task/SKILL.md`는 `opal-harness.md` §2 규칙 인덱스(직접 참조)에서 로드된다. 해당 스킬 프로세스를 따라 TASK.md를 작성한다.
...
4. **모드 플래그(`--interactive`/`--semi-agentic`/`--agentic`)를 TASK.md 헤더 `모드` 필드에 기록**한다(기본값 `semi-agentic`).
5. **[필수] `state init`을 호출하여 STATE.md를 생성한다** (LLM 직접 작성 금지, `harness/state-template.md` §[MUST] 블록).
   ...
   - `--task-title` 생략 시 task-path 마지막 디렉토리명 / `--next-action` 생략 시 `"PLAN 단계 진입"`(이후 advance/mark가 파이프라인 프론티어 기준 자동 갱신, 전이 시 1회성 오버라이드 가능 — 072) / 행 구성은 오케스트레이터 SKILL.md "STATE.md 도메인 치환값" 참조. 근거: `tasks/134-260501-opp-pipeline-state-tool/TASK.md` F-9 / `PLAN.md` §2.11 G-8 / §2.19.1 / §1.5 M-3
...
| v1.7 | 2026-08-09 21:00 | "스킬 영역" 절의 `op-task/SKILL.md` Read 지시를 제거하고 `opal-harness.md` §2 규칙 인덱스(직접 참조) 포인터로 대체(2홉화) + 오케스트레이터 공통 영역 중복 서술 압축(state-tool 명령 행 보존) (087) |
