...
**STATE 기록 규약**: 폴백(①②) 발동 시 STATE.md **자유 텍스트 영역**(블로커/다음 액션 — **파이프라인 행 아님, state-tool 비경유**)에 `code-scan 폴백: {사유}` 1줄을 기록한다.

[MUST] TASK §제약: "STATE.md 폴백 기록은 자유 텍스트 영역만 사용, 파이프라인 행 직접 편집 금지."
