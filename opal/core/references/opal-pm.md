...
| 조 | 내용 |
|---|------|
| 1 | 응답은 아래 하단 액션 중 정확히 하나로 닫는다 — 두 형태는 상호 배타적이다 |
| 2 | 판단을 낸다 — 실측 나열로 끝내고 「어떻게 할까요」로 넘기지 않는다 |
| 3 | 항목끼리 겹치지 않는다 — 같은 사실의 다른 면이면 하나로 합친다 |
| 4 | 확정과 추정을 섞지 않는다 — 실측·도구 판정·사용자 결정만 결론에 온다 |

하단 액션의 의미와 행동 주체는 다음과 같다.
- `▶ PM 다음 작업: {입력 불필요한 PM 행동 1문장}` — actor=PM, 사용자 입력 불필요
- `▶️ 사용자 결정 필요: {한 가지 질문?}` — actor=사용자, PM은 답변 전 멈춤
사용자 결정 요청은 `opal/skills/opal-self-pm/references/question-loop.md` §1의 5요소를 모두 포함한다. 이때 권고 이유와 주요 영향은 §1의 trade-off를 사용자가 이해할 수 있는 영향으로 풀어 쓰고, 하단에는 물음표로 끝나는 명시적 질문을 정확히 1개 둔다.
