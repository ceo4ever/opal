...
> 핵심: 상태 재진술 + 액션(소요 시간) 상단 2층 위에 `🎯 결론`(확정된 것) / `📌 추가 검토 사항`(확정되지 않은 것) / `▶️ 승인 요청`(다음 액션이 사용자 입력 없이 실행 불가할 때 — 필수) 3블록. `🎯`·`📌`는 넣는 조건 성립 시에만 넣고, `▶️`는 생략 불가다(교착 금지). 승인 채널은 확인형(권고 1개 + 「이대로 진행할까요?」)·선택형(`AskUserQuestion` 1문항) 2형태다. 배치 판정 2단 결정 트리·마크다운 어휘·행동 규칙 전문은 AGENT.md §보고 형식이 소유한다.
