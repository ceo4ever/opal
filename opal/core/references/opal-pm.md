...
| v2.6 | 2026-09-06 13:18 | §8 「보고 형식」 본문을 안내 1줄로 교체 — AGENT.md §보고 형식 전면 제거(108)에 따라 인용하던 골격 4층 규범 원문을 삭제. 게이트 3종 보고 양식은 `opal-harness-semi-agentic.md` §10, 세션 첫 응답은 §15가 소유하며 그 외 보고 형식은 프레임워크가 규정하지 않음을 명시. 헤딩·절 번호는 무변경 (108) |
| v2.7 | 2026-09-08 16:01 | §8 「보고 형식」 최소 규범 신설 — 108 전면 제거 후 무규범이던 3유형(질의응답·분석 / 제안·경보 / 정정)에 4조 + 템플릿 1종을 도입. 적용 범위를 PM 응답으로 한정(Phase B 로드 — 비서 tier 미적용)해 상시 로드 코어 증가 0줄. 개수·분량 상한은 템플릿 주석이 단독 소유(조문 재기재 금지), 절 길이 35줄 상한 + 「문장 추가 대신 조 교체」 개정 잠금으로 규범 증식 나선 차단. 헤딩·절 번호 무변경 (L2 직접 수정) |
