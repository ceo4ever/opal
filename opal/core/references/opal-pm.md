...
> 핵심: 상태 재진술 + 액션(소요 시간) 상단 2층 위에 `🎯 결론`(확정된 것) / `📌 추가 검토 사항`(확정되지 않은 것) / `▶️ 승인 요청`(사용자 결정 필요 — `AskUserQuestion` 1문항) 3블록. 3블록은 각자의 넣는 조건이 성립할 때만 넣는다(고정 아님). 배치 판정 2단 결정 트리·마크다운 어휘·행동 규칙 전문은 AGENT.md §보고 형식이 소유한다.
