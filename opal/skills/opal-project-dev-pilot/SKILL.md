...
PRD → TRD → 로드맵 → 태스크 순차 실행까지, PM 검수와 사용자 확정을 거치며 체계적으로 진행한다.

## 설계 원칙

- **PM이 직접 작성한다**: 모든 산출물(PRD, TRD, ROADMAP)은 알투가 PM으로서 직접 작성한다. 플레이스홀더 치환이 아니다.
- **PM 검수 → 사용자 확정**: 초안 작성 → PM 자체 검수(가이드 체크리스트 1:1 대조) → 미달 시 재작성(최대 1회) → 통과 → 사용자 검토 요청. 사용자은 PM이 통과시킨 결과물만 검토한다.
...
사전 조건 체크 → 태스크 생성 (TASK.md + STATE.md)
  → Phase 1: PRD 작성 → PM 검수 → 사용자 확정 → docs/PRD.md
  → Phase 2: TRD 작성 → PM 검수 → 사용자 확정 → docs/TRD.md
  → Phase 3: 로드맵 수립 → PM 검수 → 사용자 확정 → docs/ROADMAP.md
...
사용자 대화와 프로젝트 분석을 기반으로 PRD 초안을 작성한다.
...
5. **통과 시**: 사용자 검토 요청으로 넘어간다

### 1-4. 사용자 확정

```
---
[PRD] PM 검수 통과 — 사용자 검토 요청
...
| 피드백 | 피드백 반영 → PM 재검수 → 사용자 재검토 |

### 1-5. 사용자 확정 후 후속 조치 (필수)
...
### 4-0. 의존성 최신 검증 (Phase 4 시작 전)

TRD에서 확정한 기술 스택 버전이 아직 유효한지 재검증한다.
TRD 확정과 Phase 4 시작 사이에 시간이 경과했을 수 있으므로, 구버전/deprecated 위험을 방지한다.
...
Phase 4 시작 전:
  ...
  2. 핵심 라이브러리별 웹 검색:
     ...
     - 보안 취약점 패치 여부
  3. 변경 필요 시:
     → 사용자에게 보고: "TRD 확정 이후 {라이브러리}에 변경이 있습니다"
     → TRD + ARCHITECTURE.md 갱신 후 진행
  4. 변경 없으면 그대로 진행
```
...
사용자은 PM이 통과시킨 결과물만 검토한다. 품질이 낮은 초안이 사용자에게 올라가지 않는다.

### PM 검수 → 학습 루프 연결

PM 검수 로그가 쌓이면서 **반복 패턴이나 인사이트**가 발견되면 PM 학습 루프로 승격한다.

```
PM 검수 로그 축적
  │
  ▼
패턴 감지 (같은 유형의 Fail이 반복되거나, 새로운 판단 기준이 필요)
  │
  ▼
사용자에게 제안: "이런 패턴이 보입니다. 확정 기준으로 등록할까요?"
  │
  ├─ 사용자 승인 → .opal/AGENT.md "확정 기준"에 추가
  │              다음부터 자동 적용 (재발 방지)
  │
  └─ 사용자 거절 → 검수 로그에만 유지
```
...
**승격 기준**:
- 동일 유형의 Fail이 **2회 이상** 반복될 때
- 사용자 피드백에서 **새로운 원칙**이 도출될 때
- PM이 판단 근거를 설명하기 어려운 상황이 **반복**될 때
...
| Phase | 산출물 | 상태 |
|-------|--------|------|
| 1-PRD | docs/PRD.md | {미시작 / 작성 중 / PM 검수 / 사용자 검토 / 확정} |
| 2-TRD | docs/TRD.md | {미시작 / 작성 중 / PM 검수 / 사용자 검토 / 확정} |
| 3-ROADMAP | docs/ROADMAP.md | {미시작 / 작성 중 / PM 검수 / 사용자 검토 / 확정} |
...
| PM 검수 Fail | PM 검수 로그에 지시 내용 기록 |
| PM 검수 Pass | PM 검수 로그에 Pass 기록 |
| 사용자 확정 | Phase 상태: 확정, 다음 Phase로 전환 |
