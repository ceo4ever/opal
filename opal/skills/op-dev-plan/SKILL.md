...
- **[MUST] 재도출 금지**: `[결정]` 태그 항목은 새로 설계하지 않고, 항목별 3값 판정만 수행한다.
- **3값 판정 포맷**: PLAN.md 최상단에 `## 확정 입력 판정` 표(`항목 | 판정 | 근거`)를 둔다. 판정값은 `유효` / `수정필요` / `사실오류`. `수정필요`·`사실오류`는 근거 인용이 필수다(`citation-rules.md` §2 형식).
- **`사실오류` 경로**: 해당 항목의 확정 지위를 박탈하고 정상 설계 경로로 복귀한다. 동시에 소유자에게 보고한다 — 산출물과 완료 보고 양쪽에 기재.
...
- **관련 파일 맵** (영역 | 경로 | 역할 | 변경 유형) — 6영역: FE/BE/DB/환경/배치/공통
...
| 항목 | 판정 | 근거 |
|------|------|------|
| {TASK.md `[결정]` 항목} | 유효 / 수정필요 / 사실오류 | {`수정필요`·`사실오류`는 인용 필수, 그 외 `-`} |

> TASK.md에 `[결정]` 태그 항목이 없으면 "해당 없음"으로 표기.
...
#### 2.N.2 현재 구현
{핵심 로직 흐름 요약 + 함수/클래스 시그니처 + 외부 의존성 — 함수 본문 전사 금지, `경로:줄번호` 인용으로 대체(citation-rules.md §2.2)}
...
| v2.7 | 2026-08-21 | 확정 입력 소비 규약(재도출 금지·3값 판정·사실오류 경로) 신설 + PLAN.md 출력 형식에 `## 확정 입력 판정` 표 추가 + §2.N.2 현재 구현 절 원문 전사 금지로 교체 (098) |
| v2.8 | 2026-08-24 22:40 | 관련 파일 맵 컬럼 표기를 `변경 유형`(공백 포함)으로 통일 — SSOT `opal/core/references/harness/analysis-core.md:102`와 문자열 불일치 선재 결함 정정. 소유자 승인으로 범위 편입 (101) |
