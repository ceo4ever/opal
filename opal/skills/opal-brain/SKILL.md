...
### 답변 구조 — 적응형 마크다운 계층 (대화형·read-only 공통)

query 답변(대화형 `//opbr ask` · 비대화형 `--read-only` **공통**)은 **레이아웃을 먼저 추론한 뒤 그 골격에 내용을 채운다.** 레이아웃은 **질의 유형(고정 택소노미)이 아니라 주입된 하위 문서의 실제 관계 구조에서 도출**한다(content-driven) — 표면적으로 "비교니까 표"처럼 질의 문구만 보고 단정하지 않고, 실제로 Read한 페이지 내용이 어떤 관계(여정·병렬·값 등)를 담고 있는지로 판정한다.

본 절 규칙은 대화형·read-only **공통**이며, read-only는 이 골격을 JSON `answer` 값에 이스케이프해 담는다(§비대화형 read-only 모드 출력 계약).

**내부 워크플로우 — 6단계 (1~4는 비출력 내부 사고, 5~6만 출력)**

| # | 단계 | 하는 일 | 출력 |
|---|------|---------|------|
| 1 | 질의 분해 | 하위 주제 수 식별(단일/복합). 고정 질의유형 택소노미는 폐기 | 비출력(내부) |
| 2 | 지식 수집 | brain-tool search → score 상위 선별 페이지 Read 주입(내용만 확보) | 비출력(내부) |
| 3 | 구조 분석 | 주입된 내용의 관계를 **문서 근거로** 발견(여정/병렬/계층/조건). 관측 축 값을 채운다 | 비출력(내부) |
| 4 | 레이아웃 설계 | 관측 축 → 후보 매핑 규칙으로 골격을 선택(표면 추측 금지, content-driven) | 비출력(내부) |
| 5 | 내용 합성 | 확정된 골격에 근거·인용을 채운다 | 출력 |
| 6 | 자기검증 | 인용 정합·구조 준수·(read-only) JSON 이스케이프를 확인한 후 출력 | 출력 |

**관측 축 6종** — 3단계(구조 분석)에서 문서 근거로 채우는 신호:

| 관측 축 | 신호 (무엇을 보는가) | → 후보 |
|---------|------|--------|
| 여정/라이프사이클 | 참여·상태 전이 등 시간축 서사 | Flow/단계 |
| 순서성 | 선후·의존 순서 | Flow/단계 |
| 값 보유 | 항목마다 대응 값/속성이 다수 | 표(GFM) |
| 병렬성 | 대등·비종속 항목이 다수 | 그룹핑(헤딩+불릿) |
| 주제 수 | 독립 하위 주제 2개 이상 | 복합(섹션 헤딩 분할) |
| 분량 | 답변 총량이 적음 | Flat(과잉 헤딩 억제) |

**후보 레이아웃 5종** — 4단계(레이아웃 설계)에서 채택하는 골격:

- `Flat` — 리드 문단(+`**볼드**`), 헤딩 없음. 분량이 적고 단일 주제일 때.
- `그룹핑(헤딩+불릿)` — `## 섹션` + `-` 불릿. 대등한 병렬 항목일 때.
- `Flow/단계(번호)` — `## 섹션` + `1.` 번호. 여정·순서가 드러날 때.
- `표(GFM)` — 마크다운 표. 값 보유(항목×속성)가 드러날 때.
- `복합(섹션 조합)` — 위 후보들을 `## 섹션`으로 조합. 독립 주제 수가 2개 이상일 때.

**판정 규칙**: 1차로 **관측 축 → 후보 매핑**을 적용해 축이 가리키는 후보를 채택한다. **여러 후보가 동점일 때만** tie-break을 적용하며, 기준은 **더 단순한 쪽**(과잉 구조 억제)이다. 전면 가중합 스코어링은 채택하지 않는다(self-scoring 편향·과잉 무게 우려). 축이 충돌하는 예 — 값 보유(→표)와 여정(→Flow)이 동시에 관측되면 문서 근거상 지배적인 서사 축을 우선하고, 그래도 동점이면 더 단순한 쪽을 택한다.

**불변 가드 3종**:

- **G1 비출력 내부 사고** — 1~4단계는 답변에 노출하지 않는다. 최종 출력은 답변(또는 read-only는 JSON 코드펜스 하나)만이며, 레이아웃 설계 개요나 단계 로그를 출력하지 않는다.
- **G2 호출 1회 유지** — 단계가 6개로 늘어도 claude subprocess 호출은 **1회**다. 단계는 별도 호출이 아니라 단일 세션 내부의 사고 흐름이다. 콜드 스타트 ~56s의 latency를 배증시키지 않는다.
- **G3 read-only JSON 이스케이프** — read-only의 최종 출력은 JSON 코드펜스 **하나**다. 펜스 밖 raw 마크다운을 출력하지 않는다(citations 유실 방지). §비대화형 read-only 모드의 [MUST] JSON 이스케이프 계약을 그대로 유지한다.

**예시**:

- **예1 — "방문형 캠페인 정책 설명" → Flow/단계.** 표면상 "정책 설명이니 표"로 보이지만(오판), 주입된 캠페인 문서를 Read해보면 실제로는 **참여 여정(라이프사이클)** 정책이 시간축 서사로 드러난다. 여정/순서 축이 지배하므로 Flow를 채택한다.
- **예2 — "미션 유형 + 정책" → 복합.** 독립된 하위 주제가 2개(미션 유형 / 정책)이므로 주제 수 축이 지배해 복합을 채택한다. 각 섹션 내부는 다시 축으로 결정한다(미션 유형=값 보유가 드러나면 표, 정책=여정이 드러나면 Flow).

**표현·가독성 규율** — 위 6축 판정과 별개로 항상 지킨다:

- **항상**: 리드 문단 1~2문장으로 핵심을 먼저 요약한다.
- 코드 식별자·코드값은 인라인 코드(예: `` `POL-300` ``)로만 병기한다(확정기준 #2 — 자연어 우선).
- 짧은 답에 과한 헤딩을 강제하지 않는다 — 위 6축의 "분량 → Flat"에 흡수된 규칙이다.
- **항목 내부 다문장 분해**: 번호·불릿 항목의 설명이 2문장 이상이면, 제목 줄 아래 하위 불릿(`-`)으로 문장을 나눠 다음 라인으로 내린다. 한 항목 안에 여러 사실을 한 줄로 이어붙이지 않는다.
- **1라인 1내용**: 한 줄에는 하나의 사실만 담는다(터미널·FE 렌더 가독성). 근거는 보고형식 "1불릿 1문장"과 동일 사상이며, Flow·그룹핑·복합 등 전 후보에 공통 적용한다.

> **출력 형식 차이**: 대화형 `ask`는 이 구조를 **답변 텍스트로 직접** 출력한다. `--read-only`는 이 구조를 **JSON answer 값에 이스케이프**하여 담는다(§비대화형 read-only 모드 출력 계약).
...
- **출력 계약**: 응답은 아래 JSON 코드펜스 **하나만** 출력한다(JSON 펜스 밖 산문·보고형식·후보목록 없이). 인용은 합성에 실제 사용한 페이지로 채운다.
  ```json
  {"answer":"<최종 답변 (적응형 마크다운)>","citations":[{"page":"<페이지 경로>","title":"<제목>","type":"<concept|entity|flow|synthesis|term>"}]}
  ```
- **answer 구조**: answer 값은 **§답변 구조 — 적응형 마크다운 계층**(query 공통 절)을 따른다.
- **[MUST] JSON 이스케이프**: 전체 응답은 반드시 **유효한 JSON 코드펜스 하나**여야 한다. answer의 마크다운 개행은 `\n`, 큰따옴표는 `\"`로 이스케이프하여 **JSON 문자열 값 안에** 담는다. **마크다운(헤딩·표·불릿)을 JSON 펜스 밖으로 raw 출력하지 않는다** — raw로 내보내면 소비자가 인용(citations)을 잃는다.
