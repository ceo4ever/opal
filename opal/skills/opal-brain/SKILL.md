...
query 답변(대화형 `//opbr ask` · 비대화형 `--read-only` 공통)은 소비자가 마크다운으로 렌더(터미널·FE 모두)하므로, 내용 성격에 맞는 구조로 작성한다. **계층 깊이는 내용량에 따라 조절하며, 짧은 답변에 과한 헤딩을 강제하지 않는다.**

- **항상**: 리드 문단 1~2문장으로 핵심을 먼저 요약한다.
- **단순 답변**: 리드 문단 + 핵심어 `**볼드**` 강조.
- **나열형**: `## 섹션` 헤딩 + 불릿(`-`) — 2단.
- **그룹핑되는 나열**: `## 섹션` + 번호(`1.`) 그룹 + 하위 불릿 — 3단.
- **비교형**: 마크다운 표(GFM).
