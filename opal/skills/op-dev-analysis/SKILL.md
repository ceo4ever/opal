...
- **[MUST] 재도출 금지**: `[결정]` 태그 항목은 새로 설계하지 않고, 항목별 4값 판정만 수행한다.
- **4값 판정 포맷**: 산출물 최상단에 `## 확정 입력 판정` 표(`항목 | 판정 | 근거`)를 둔다. 판정값은 `유효` / `승계` / `수정필요` / `사실오류`. `수정필요`·`사실오류`는 근거 인용이 필수다(`citation-rules.md` §2 형식). 상류(브레인·과거 산출물 등)에서 이미 대조 확인된 항목은 재확인하지 않고 `승계`로 기재한다.
...
| 항목 | 판정 | 근거 |
|------|------|------|
| {TASK.md `[결정]` 항목} | 유효 / 승계 / 수정필요 / 사실오류 | {`수정필요`·`사실오류`는 인용 필수, 그 외 `-`} |
