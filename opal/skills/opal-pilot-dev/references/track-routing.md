...
- 사용자가 수락하면 기존 TASK·ANALYSIS 산출물과 현재 **effective mode**를 인계해 `opds --interactive|--semi-agentic|--agentic` 중 판정값에 대응하는 명시 플래그로 PLAN에 진입한다. 이 전달은 mode 변경이 아니며 파이프라인 강제 재초기화를 금지한다.
