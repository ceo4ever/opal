...
의존성이 없는 액션을 동시에 실행하여 프로젝트 개발 속도를 향상한다. 각 액션을 독립된 git worktree에서 격리 실행하고, 완료 후 순차 머지하여 통합한다.
...
머지 완료 시 STATE.md에 이력을 기록한다 (하네스 병렬 실행 State 구조 준수).
