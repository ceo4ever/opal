...
|-----|------|------|------|------|---------|----------|------|------|------|
| ACT-001 | {name} | Group 1 | — | ✅ | ✅ | ✅ | ✅ 완료 | YYYY-MM-DD | YYYY-MM-DD |
| ACT-002 | {name} | Group 2 | ACT-001 | 🔄 | ⬜ | ⬜ | 🔄 진행 중 | YYYY-MM-DD | — |
| ACT-003 | {name} | Group 2 | ACT-001 | ⬜ | ⬜ | ⬜ | ⬜ 대기 | — | — |
| ACT-004 | {name} | Group 3 | ACT-002, ACT-003 | ⬜ | ⬜ | ⬜ | ⬜ 대기 | — | — |
...
PM은 각 ACT 완료 후 다음 순서로 직접 검증을 실행한다:

ACT-N 완료 (코드)
  → L1: tsc --noEmit (lint + type check)
  → L2: pnpm build
    → Pass → ACT 목록 코드 ✅, L1 ✅, L2 ✅, 상태 ✅
...
|----------|-----------|--------|
| L1 lint/type | 제한 없음 | — |
| L2 build | 2회 | 소유자 에스컬레이션 |

검증 시점:
- 매 ACT 완료 후: 해당 ACT 변경이 빌드를 깨뜨리지 않는지
- 전체 ACT 완료 후: 최종 통합 빌드 확인
...
| 이벤트 | 갱신 내용 |
|--------|----------|
| ACT 시작 | ACT 상태 ⬜ → 🔄, 시작 날짜 기록 |
| ACT TEST.md 확인 (Pass) | ACT 상태 🔄 → ✅, TS 상태 → Green |
| ACT TEST.md 확인 (Fail) | ACT 상태 유지 🔄, TS 상태 → Fail, 재시도 루프 진입 |
| ACT L1 통과 | 코드 ✅, L1 lint ✅ 갱신 |
| ACT L2 통과 | L2 build ✅, 상태 ✅, 완료 날짜 기록 |
| ACT L1/L2 실패 | L1 lint ❌ 또는 L2 build ❌ 갱신 |
