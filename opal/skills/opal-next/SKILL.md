...
name: opal-next
description: |
  **재진입 가이드 스킬** — 현재 OPAL 환경 상태를 진단하여 사용자에게 다음 액션을 권유한다. //next 등으로 호출된다 — '다음에 뭐 해야' 류 자연어 포함.
  반드시 이 스킬을 사용해야 하는 상황: 사용자가 //next 입력 시 / 첫 사용자가 무엇을 해야 할지 모를 때 / 온보딩 이후 재진입이 필요할 때.
triggers:
  - "//next"
  - "다음에 뭐 해야"
  - "어디서부터 시작"
...
# opal-next — OPAL 재진입 가이드

## 목적

사용자가 OPAL 환경에서 어디서 막혔는지 진단하고, 다음 한 가지 액션을 명확하게 권유한다.

상세 진단·라우팅 흐름은 `references/next-flow.md`를 Read하여 참조한다.
...
상세 라우팅 흐름은 `references/next-flow.md`에 기술되어 있다.
...
| v2.0.0 | 2026-06-21 13:58 | 개명 (030) — opal-start → opal-next, //start alias·트리거 제거, references→next-flow.md |
