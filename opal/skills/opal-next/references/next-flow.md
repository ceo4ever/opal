# opal-next 진단·라우팅 흐름 가이드

> 로드 시점: opal-next/SKILL.md Step 1·2 실행 시
...
`//next`가 호출되면 아래 순서로 환경 상태를 점검하고 라우팅 결정을 내린다.
...
가능한 작업:
  ...
  - 프로젝트 폴더로 이동 후 //next 재실행하면 PM 모드로 진입 가능
```
...
프로젝트 폴더이지만 OPAL 환경이 초기화되지 않았습니다.
  ...
  - 초기화 후 //next 재실행으로 정상 PM 모드를 확인하세요.
```
...
| v2.0.0 | 2026-06-21 13:58 | 개명 (030) — start-flow.md → next-flow.md, //start→//next |
