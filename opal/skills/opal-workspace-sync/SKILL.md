...
**선언이 있으면 선언이 권위다.** 디스크에 있어도 선언에 없으면 `undeclared`로 보고되고 최신화되지 않는다 — 새 레포를 clone한 사용자가 "왜 pull이 안 되나"로 막히는 지점이므로, `undeclared`를 보고할 때 **선언에 추가해야 sync 대상이 된다**는 사실을 함께 알린다.
