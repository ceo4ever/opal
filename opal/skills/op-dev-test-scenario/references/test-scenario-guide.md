...
3. **목표 달성(채택 관점) 검증** — 리스크 가설(파괴 관점)만으로는 "핵심 목표를 검증하는 시나리오 자체가 도출되지 않는" 결함을 못 잡는다(070 사건 근거, → `opal/core/references/harness/scenario-gate.md` §1). TASK.md 요구사항(R)·목표 문장·(교체형 목표인 경우) 채택/잔존 기준을 도출 입력에 병행해 목표달성·채택/잔존·경계 관점 시나리오를 확보한다. 판정 규칙 SSOT는 `scenario-gate.md`.

---

## 작성 프로세스

### Step 1: 도출 입력 2계열 Read (Block A: TASK 유래 / Block B: PLAN 유래)

시나리오 도출 입력은 **원천에 따라 2계열**로 나뉘며, 계열마다 커버하는 루브릭 축과 착수 가능 시점이 다르다. 트랙 규칙 SSOT: `opal/core/references/harness/red-first.md` §1.6.
...
> 루브릭 6축의 **정의·판정 주체**는 `opal/core/references/harness/scenario-gate.md` §2가 SSOT다 — 본 표는 계열↔축 매핑만 소유한다.

선작성 트랙을 쓰지 않는 경우 Block A → Block B를 연속 수행한다(순차 경로 = 현행 동작, 결과 동등).

#### Block A. 채택 관점 입력 (TASK 유래 — 선작성 가능)

**[MUST] 목표/채택 관점 도출 입력 병행**: PLAN 가설 표(파괴 관점 — Block B)에 더해 아래를 함께 도출 입력으로 삼는다(목적 3 "목표 달성(채택 관점) 검증", → `scenario-gate.md` §2 ①⑤⑥):
- TASK.md **요구사항(R)** 전체 목록 (§4 매핑 표에서 전 R 커버 여부 확인) — 대응 축 ② (→ `scenario-gate.md` §2)
- TASK.md **목표 문장** (사용자/운영 계층에서 검증 가능한 목표달성 시나리오 도출 근거) — 대응 축 ① (→ `scenario-gate.md` §2)
- (교체형 목표인 경우) TASK.md **채택/잔존 기준** (구형 잔존0·신형 채택 검증 시나리오 도출 근거) — 대응 축 ⑤ (→ `scenario-gate.md` §2)

**[MUST] ⑥경계·부정 축 동시 도출**: Block A만으로 도출할 때 AC를 그대로 옮긴 "당연한 시나리오"만 남으면 self-confirming 구조로 퇴행한다(근거: `scenario-gate.md` §1 "도출 엔진의 관점 편향"). Block A 산출에는 **⑥경계·부정 축 시나리오를 최소 1건 포함**한다 — 각 R의 경계값·부정 경로(실패·거부·미충족 입력)를 R별로 1회 이상 질의하여 도출한다.

**선작성 트랙 사용 시 [MUST] 3항목**:
1. 초안은 별도 임시 파일을 만들지 않고 **TEST-SCENARIO.md 본문에 직접** 작성한다 (태스크 폴더 밖 산출물 금지 / STATE 행 신설 금지).
2. Block B 보강이 필요한 지점에 보강 대기 마커 `<!-- PENDING-BLOCK-B -->` 를 남긴다. 최소 2곳 — §1 리스크 가설 표, §4 매핑 표의 "가설 ID" 열.
3. 이 시점에 목표-커버 게이트를 호출하지 않는다 (→ `opal/core/references/harness/scenario-gate.md` §4).
...
PLAN.md §1.2 기능 목록에서 F-NNN 전체를 Read하여 §4 매핑 표의 전 F 커버 여부를 확인한다 (→ `scenario-gate.md` §2 ③).

**[MUST] 보강은 additive-only가 아니다**: 선작성 초안이 있는 경우 Block B 보강을 시나리오 추가만으로 끝내지 않는다. 초안의 각 시나리오를 H-N·F-NNN과 대조하여 **중복·과잉·PLAN 설계와 어긋나는 시나리오를 수정 또는 삭제**한다. 선작성 초안과 PLAN 설계의 불일치는 그 자체가 조기 경보 신호이므로 PM Gate에서 표면화한다.

**보강 완료 판정 3조건**:
1. 보강 대기 마커 잔존 0건. **검사 전 각 줄의 인라인 백틱(`` ` ``) 구간을 제거한 뒤 검사한다** — 마커 리터럴을 설명하는 규칙 문서·시나리오 문서 본문의 백틱 표기까지 잡히면 마커를 설명하는 문서 자신이 영구 FAIL하는 메타-순환 오탐이 발생한다. 선례: `opal/tools/state-tool/state_tool.py:2010,2025`가 034에서 동일 문제를 인라인 백틱 구간 제거(`re.sub`) 전처리로 해소했다.
2. §1 리스크 가설 표에 PLAN.md H-N 전건 전재.
3. §4 매핑 표의 모든 시나리오 행에 가설 ID·검증 계층이 채워짐.

목표-커버 게이트는 이 3조건 충족 상태에서만 호출한다 (→ `opal/core/references/harness/scenario-gate.md` §4).

#### 선작성 대상이 아닌 Step

[MUST] Step 2(데이터 설계)·Step 3(계층 결정)은 **변경 영역 의존**이므로 선작성 대상이 아니다 — 계층 결정 규칙 표(Step 3)가 "변경 영역"을 행 축으로 쓰고, Step 2 §2.1 사전 조건 데이터는 PLAN 파일 변경 계획 확정 후에만 채울 수 있다. 두 Step은 PLAN 확정 후 Block B와 함께 수행한다. 근거: (→ D-4) 본 문서 §작성 프로세스 Step 2·Step 3 / TASK.md D-6.
...
> 위반 시 PM Gate FAIL (grep 감지).
> 대안: 실 fixture / factory / seed 데이터 사용.
> **또한 검증 대상 구현 자체를 목업/가짜 응답으로 대체한 채 시나리오를 통과시키지 않는다** — 지시된 실연동(API/DB)은 실제로 호출·검증한다. 실연동 검증이 불가하면 통과 처리하지 말고 BLOCKED로 표면화한다.
...
테스트 파일은 태스크 폴더가 아닌 **대상 모듈에 붙인다(모듈 미러링)**:
- [MUST] 모듈 1개 = 테스트 파일 1개. 후속 태스크는 기존 파일에 케이스 추가 (새 파일 분리 금지).
- [MUST] 케이스명 프리픽스 포맷: `[T{NNN}/L{계층}-{AC}]` (예: `[T016/L1-AC1]`). 태스크 번호·계층·AC ID를 추적 가능하게 한다.
- 테스트 파일 @header에 선택 필드 `task`(태스크 번호)·`scenarios`(연결 S-ID 목록)를 작성한다 (header-rules.md 참조).
- §4 매핑 표의 "테스트 파일:케이스" 열에 `{테스트 파일 경로}:{케이스명}` 형식으로 연결 추적한다.

### Step 4-c: 공개 인터페이스 검증 규율 (F-007)

[MUST] 테스트는 내부 구현/private 구현 세부에 결합하지 않고 **공개 인터페이스·관찰 가능 행위(반환값/exit code/관측 출력)**로 검증한다.
- 내부 함수·private 속성·구현 세부에 직접 결합하는 테스트 작성 금지.
- state-tool 자기적용 예시: `cmd_verify` 내부가 아닌 exit code/JSON 응답으로 검증.
...
- [ ] (선작성 트랙 사용 시) `<!-- PENDING-BLOCK-B -->` 마커가 0건인가? (인라인 백틱 구간 제거 후 grep 확인)
