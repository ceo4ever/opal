...
### Step 1: PLAN 가설 표 + TASK 목표/R/채택 기준 Read
...
**[MUST] 목표/채택 관점 도출 입력 병행**: 위 PLAN 가설 표(파괴 관점)에 더해 아래를 함께 도출 입력으로 삼는다(목적 3 "목표 달성(채택 관점) 검증", → `scenario-gate.md` §2 ①⑤⑥):
- TASK.md **요구사항(R)** 전체 목록 (§4 매핑 표에서 전 R 커버 여부 확인)
- TASK.md **목표 문장** (사용자/운영 계층에서 검증 가능한 목표달성 시나리오 도출 근거)
- (교체형 목표인 경우) TASK.md **채택/잔존 기준** (구형 잔존0·신형 채택 검증 시나리오 도출 근거)
