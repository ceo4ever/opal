...
> **사용법**: 이 내용이 `~/.gemini/GEMINI.md`에 GEMINI HARDENING 마커 기반으로 자동 삽입된다.
...
- **모드별 규칙**: 세션에 로드된 서브 하네스를 따른다
  - `opal-harness-interactive.md` → 각 단계 완료 후 소유자 승인 게이트
...
현재 상태: {현재 단계}
조치: 즉시 중단 → 미충족 조건 충족 → 소유자 승인 요청 (interactive) 또는 에스컬레이션 (agentic)
```
