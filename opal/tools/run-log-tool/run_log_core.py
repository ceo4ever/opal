...
def validate_event(event):
    ...
    if event_type == "activity" and data is not None:
        ...
        # CONTRACT §1.3 "PM activity의 사건 고유 payload 축 폐쇄 목록" — 조합 A4
        # (actor.kind=PM ∧ provenance.type=direct)의 activity에서 data는 kind 1개
        # 키만 허용한다. 값 enum은 위에서 이미 판정했으므로, 단일 키인데 kind가 4종
        # 밖인 경우는 여기까지 오지 않고 기존 사유로 거부된다. 적용 조건은 A4에
        # 한정하며(D-10) A1(adapter)·A8(import)은 provenance.type이 direct가 아니어서
        # 이 조건식으로 자연 배제된다 — 별도 예외 분기를 두지 않는다.
        if actor.get("kind") == "PM" and provenance.get("type") == "direct":
            extra_data_keys = set(data.keys()) - {"kind"}
            if extra_data_keys:
                return err("schema_invalid",
