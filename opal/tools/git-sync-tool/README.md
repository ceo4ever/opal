...
**선언을 도입하면 선언이 권위가 된다.** 디스크에 있어도 선언에 없으면 `undeclared`로 보고되고 pull하지 않는다. 즉 **새 레포를 clone해도 선언에 넣기 전까지는 최신화되지 않는다.** 선언 파일을 만드는 순간 생기는 운영상 변화이며, 드리프트를 드러내기 위해 의도한 동작이다. 선언 없는 프로젝트는 종전대로 디스크 발견 기준으로만 동작한다.
