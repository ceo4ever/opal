...
@header {
  ...
  "description": "Claude Code PostToolUse 릴레이 헬퍼 — stdin(PostToolUse 이벤트 JSON)을 파싱해 Bash·state-tool(init/advance/mark/block) 호출을 필터하고, 도구 stdout에서 build_todo_mirror 페이로드를 추출한 뒤 hookSpecificOutput.additionalContext(지시문+payload)로 세션에 결정론 주입한다. 비Bash·비state-tool·페이로드 부재·파싱 실패 등 전 경로에서 무출력 exit0 fail-safe(DEC-9) — 정상 도구 흐름을 절대 차단하지 않는다. 표준 라이브러리만(json/sys/shlex). extract_history_link로 history_link 리마인더를 병존 릴레이한다 — build_additional_context에 선택 인자 history_link가 있으면 reminder를 기존 todo 미러 지시문 뒤에 병존 첨부하며, payload 단독 부재 시에도 reminder만으로 출력한다(_extract_payload가 extract_todo_mirror/extract_history_link의 공용 내부 구현). extract_gate_checklist로 gate_checklist 페이로드도 동일하게 병존 릴레이한다 — build_additional_context의 선택 인자 gate_checklist가 있으면 reminder+checklist json을 기존 todo_mirror/history_link 뒤에 병존 첨부한다. main()은 todo_mirror·history_link·gate_checklist 3-페이로드를 모두 추출해 셋 다 없을 때만 무출력한다(fail-safe DEC-9 불변).",
  "exports": [
