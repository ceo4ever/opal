...
@header {
  ...
  "description": "todo_mirror_hook.py 단위 테스트 (TS-010~013). 합성 stdin JSON으로 (1)state-tool advance+payload→additionalContext 주입 (2)비state-tool/비Bash Bash 명령→무출력 exit0(H-6) (3)stderr 경고 혼입 stdout에서 마지막 JSON 라인 payload 추출(H-5) (4)payload 부재·깨진 JSON(stdin/stdout)→무출력 exit0 fail-safe(DEC-9) 를 검증한다. 스크립트 subprocess 실행(exit0 실증) + 순수 함수 직접 호출 병행. 표준 라이브러리만. TestHistoryLinkRelay(TS-8~TS-10)는 stdout의 todo_mirror+history_link 동시 존재 시 reminder 원문이 기존 todo 미러 지시문과 병존 릴레이되는지·history_link 부재 시 기존 동작이 불변인지(대조군으로 미주입의 선택성 확증)·history_link 단독일 때도 exit0+reminder 주입되는지를 스크립트 subprocess 실행으로만 검증한다(mock 없음). TestGateChecklistRelay(TEST-SCENARIO S-18~S-19)는 gate_checklist 단독 주입(S-18)과 todo_mirror·history_link·gate_checklist 3종 병존(S-19)을 스크립트 subprocess 실행으로만 검증한다(mock 없음).",
