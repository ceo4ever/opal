...
test('[T083/L2-F7] TS-080: 기존 11개 테스트 스크립트 전량 GREEN (자기 자신은 재귀 방지를 위해 제외 — 082 관용)', () => {
  ...
  assert.strictEqual(OTHER_TEST_FILES.length, 13,
    `[MUST] 083 신규 파일 1개(test-shard-policy.js 자기 자신) + 107 신규 파일 1개(test-header-history.js) + 109 신규 파일 1개(test-hub-root.js)를 제외하면 기존 스크립트가 정확히 13개여야 함(109 진행 시점 기준), got ${JSON.stringify(OTHER_TEST_FILES)}`);
  const failures = [];
