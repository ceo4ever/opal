...
@header {
  ...
  "layer": "util",
  "domain": "opal-tools",
  "description": "oppl 2-루프 실행의 유한 실행 계약을 집행하는 CLI — 6개 서브 명령(config/init/admit/attempt-start/attempt-finish/show). 출력은 단일 라인 JSON + exit code만이며(opal/core/references/harness/tool-output-contract.md), 성공은 ok:true·exit 0, 거부는 ok:false·비영 exit이다. init은 state.json.run_id가 없으면 run_identity_missing으로 거부하고 run_id를 발급하지 않는다(D8/C-5). admit은 같은 lock 안에서 제안서 §6.2의 6개 검사를 수행하고 허가된 경우에만 카운터를 증가시키며 active attempt를 예약한다 — 거부 코드는 active_attempt·round_limit_exceeded·attempt_limit_exceeded·resume_limit_exceeded·budget_exceeded·timeout_limit_exceeded·no_progress·decision_required 폐쇄 집합이고, dispatch 상한 초과는 전용 코드가 없으므로 attempt_limit_exceeded에 scope 필드를 동반해 구분한다. attempt-start는 예약 토큰을 실제 attempt-id·attempt record 경로로 바인딩하고, attempt-finish만이 phase 상태 7종을 확정하는 receipt 경로다 — receipt 없는 done 기록 경로를 만들지 않는다(AC-12). T4b 3분기는 어느 경로든 task attempt와 project dispatch를 함께 차감한다(제안서 §6.4). cost_used는 terminal candidate 값을 그대로 누적하며 stream의 result 개수로 배수되지 않는다(C-10).",
  "exports": [
